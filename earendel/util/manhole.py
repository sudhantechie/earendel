...
from earendel.config.server import ManholeConfig
...
def manhole(settings: ManholeConfig, globals: dict[str, Any]) -> ServerFactory:
    ...
    rlm.chainedProtocolFactory = lambda: insults.ServerProtocol(  # type: ignore[misc,assignment]
        EarendelManhole, dict(globals, __name__="__console__")
    )
...
class EarendelManhole(ColoredManhole):
    ...
    def connectionMade(self) -> None:
        ...
        self.interpreter = EarendelManholeInterpreter(self, self.namespace)

        # this would also be a good place to add more keyHandlers.


class EarendelManholeInterpreter(ManholeInterpreter):
