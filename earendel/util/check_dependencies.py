...
"""
This module exposes a single function which checks earendel's dependencies are present
...
DISTRIBUTION_NAME = "matrix-earendel"
...
def _not_installed(requirement: Requirement, extra: str | None = None) -> str:
    if extra:
        return (
            f"Earendel {VERSION} needs {requirement.name} for {extra}, "
            f"but it is not installed"
        )
    else:
        return f"Earendel {VERSION} needs {requirement.name}, but it is not installed"
...
def _incorrect_version(
    ...
    if extra:
        return (
            f"Earendel {VERSION} needs {requirement} for {extra}, "
            f"but got {requirement.name}=={got}"
        )
    else:
        return (
            f"Earendel {VERSION} needs {requirement}, but got {requirement.name}=={got}"
        )
...
def _no_reported_version(requirement: Requirement, extra: str | None = None) -> str:
    if extra:
        ...
        return (
            f"Earendel {VERSION} needs {requirement}, "
            f"but can't determine {requirement.name}'s version"
        )


def check_requirements(extra: str | None = None) -> None:
    """Check Earendel's dependencies are present and correctly versioned.

    If provided, `extra` must be the name of an packaging extra (e.g. "saml2" in
    `pip install matrix-earendel[saml2]`).
    ...
    if extra is None:
        ...
        raise ValueError(f"Earendel {VERSION} does not provide the feature '{extra}'")
    ...
    for requirement, must_be_installed in dependencies:
        ...
        try:
            ...
            if dist.version is None:
                ...
                # https://github.com/sudhantechie/earendel/issues/12223)
