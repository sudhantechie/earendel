...
class AbstractObservableDeferred(Generic[_T], metaclass=abc.ABCMeta):
    ...
    def observe(self) -> "defer.Deferred[_T]":
        ...
        Note that the returned Deferred doesn't follow the Earendel logcontext rules -
...
def stop_cancellation(deferred: "defer.Deferred[T]") -> "defer.Deferred[T]":
    ...
    Returns:
        ...
        The new `Deferred` will not follow the Earendel logcontext rules and should be
...
def delay_cancellation(awaitable: Awaitable[T]) -> Awaitable[T]:
    ...
    Args:
        deferred: The coroutine or `Deferred` to protect against cancellation. May
            optionally follow the Earendel logcontext rules.
    ...
    Returns:
        ...
        The new `Deferred` will follow the Earendel logcontext rules if `awaitable`
        follows the Earendel logcontext rules. Otherwise the new `Deferred` should be
        wrapped with `make_deferred_yieldable`.
    """
