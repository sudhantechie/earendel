...
from earendel.util.caches.treecache import TreeCache, iterate_tree_cache_entry
from earendel.util.clock import Clock

cache_pending_metric = Gauge(
    "earendel_util_caches_cache_pending",
...
class DeferredCache(Generic[KT, VT]):
    ...
    def get(
        ...
        For symmetry with set(), this method does *not* follow the earendel logcontext
    ...
    def set(
        ...
        *original* `value`, (c) any future calls to `get()` will complete with the
        ...
        It is expected that `value` does *not* follow the earendel logcontext rules - ie,
