...
from earendel.util.caches import EvictionReason, register_cache
from earendel.util.cancellation import cancellable, is_function_cancellable
...
class ResponseCache(Generic[KV]):
    ...
    def _set(
        ...
        *deferred* should run its callbacks in the sentinel logcontext (ie,
        you should wrap normal earendel deferreds with
        earendel.logging.context.run_in_background).
    ...
    async def wrap(
        ...
        **kwargs: Any,
        ...
        follow the earendel logcontext rules and returns it.

        Otherwise, makes a call to *callback(*args, **kwargs)*, which should
        follow the earendel logcontext rules, and adds the result to the cache.
