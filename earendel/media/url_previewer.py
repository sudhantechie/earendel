...
from earendel.media._base import FileInfo, get_filename_from_headers
from earendel.media.media_storage import MediaStorage, SHA256TransparentIOWriter
from earendel.media.oembed import OEmbedProvider
from earendel.media.preview_html import decode_body, parse_html_to_open_graph
from earendel.types import JsonDict, UserID
from earendel.util.async_helpers import ObservableDeferred
...
class UrlPreviewer:
    ...
    When Earendel is asked to preview a URL it does the following:
    ...
    async def _do_preview(self, url: str, user: UserID, ts: int) -> bytes:
        ...
        if _is_media(media_info.media_type):
            ...
            # TODO: somehow stop a big HTML tree from exploding earendel's RAM
        ...
        try:
            ...
            length, headers, uri, code = await self.client.get_file(
                ...
                headers={
                    ...
                    # Open Graph metadata to crawler user agents. Omit the Earendel version
                    # string to avoid leaking information.
                    b"User-Agent": [
                        "Earendel (bot; +https://github.com/sudhantechie/earendel)"
                    ],
                },
                is_allowed_content_type=_is_previewable,
            )
        except EarendelError:
            # Pass EarendelErrors through directly, so that the servlet
            # handler will return a EarendelError to the client instead of
            # blank data or a 500.
            raise
    ...
    async def _handle_url(
        ...
        Raises:
            EarendelError if the URL is blocked.
        """
