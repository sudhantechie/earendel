...
from earendel.config._base import ConfigError, format_config_error
...
from earendel.http.additional_resource import AdditionalResource
...
from earendel.util.module_loader import load_module

logger = logging.getLogger("earendel.app.homeserver")
...
class EarendelHomeServer(HomeServer):
    """
    Homeserver class for the main Earendel process.
    """
    ...
    def _listener_http(
        ...
        # 2. Redirect to the static "Earendel is running" page.
    ...
    def _configure_named_resource(
            ...
            resources.update(
                {
                    ...
                    **build_earendel_client_resource_tree(self),
                }
            )

            if self.config.email.can_verify_email:
                from earendel.rest.earendel.client.password_reset import (
        ...
        if name == "consent":
            from earendel.rest.consent.consent_resource import ConsentResource
        ...
        if name in ["static", "client"]:
            resources[STATIC_PREFIX] = StaticResource(
                os.path.join(os.path.dirname(earendel.__file__), "static")
            )
...
def load_or_generate_config(argv_options: list[str]) -> HomeServerConfig:
    ...
    try:
        config = HomeServerConfig.load_or_generate_config(
            "Earendel Homeserver", argv_options
        )
...
def create_homeserver(
    ...
    """
    Create a homeserver instance for the Earendel main process.
    ...
    if config.worker.worker_app:
        raise ConfigError(
            ...
            "instance. Please use `python -m earendel.app.generic_worker` instead (or remove the option if this is the main process)."
        )
...
def setup(
    ...
    """
    Setup a `EarendelHomeServer` (main) instance.
