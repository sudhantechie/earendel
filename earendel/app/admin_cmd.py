...
from earendel.events.utils import FilteredEvent
from earendel.handlers.admin import ExfiltrationWriter
from earendel.server import HomeServer
from earendel.storage.database import DatabasePool, LoggingDatabaseConnection
...
from earendel.types import IEarendelReactor, JsonMapping, StateMap
from earendel.util.logcontext import LoggingContext

logger = logging.getLogger("earendel.app.admin_cmd")
    ...
    def __init__(self, user_id: str, directory: str | None = None):
        ...
        if directory:
            ...
            self.base_directory = tempfile.mkdtemp(
                prefix="earendel-exfiltrate__%s__" % (user_id,)
            )
...
def load_config(argv_options: list[str]) -> tuple[HomeServerConfig, argparse.Namespace]:
    parser = argparse.ArgumentParser(description="Earendel Admin Command")
...
def create_homeserver(
    ...
    """
    Create a homeserver instance for the Earendel admin command process.
    ...
    if config.worker.worker_app is not None:
        assert config.worker.worker_app == "earendel.app.admin_cmd"

    # Update the config with some basic overrides so that don't have to specify
    # a full worker config.
    config.worker.worker_app = "earendel.app.admin_cmd"
