...
import earendel.util.caches
from earendel.api.constants import MAX_REQUEST_SIZE
from earendel.app import check_bind_error
from earendel.config import ConfigError
from earendel.config._base import format_config_error
from earendel.config.homeserver import HomeServerConfig
from earendel.config.server import (
    ListenerConfig,
    ManholeConfig,
    TCPListenerConfig,
    UnixListenerConfig,
...
from earendel.events.auto_accept_invites import InviteAutoAccepter
from earendel.events.presence_router import load_legacy_presence_router
from earendel.handlers.auth import load_legacy_password_auth_providers
from earendel.http.site import EarendelSite
from earendel.logging.context import LoggingContext, PreserveLoggingContext
from earendel.metrics import install_gc_manager, register_threadpool
from earendel.metrics.jemalloc import setup_jemalloc_stats
from earendel.module_api.callbacks.spamchecker_callbacks import load_legacy_spam_checkers
from earendel.module_api.callbacks.third_party_event_rules_callbacks import (
    load_legacy_third_party_event_rules,
)
from earendel.types import IEarendelReactor, StrCollection
from earendel.util import EAREENDEL_VERSION
from earendel.util.caches.lrucache import setup_expire_lru_cache_entries
from earendel.util.daemonize import daemonize_process
from earendel.util.gai_resolver import GAIResolver
from earendel.util.rlimit import change_resource_limit
...
def register_sighup(
    ...
    **kwargs: P.kwargs,
    ...
    Args:
        homeserver_instance_id: The unique ID for this Earendel process instance
...
def unregister_sighups(homeserver_instance_id: str) -> None:
    """
    Unregister all sighup functions associated with this Earendel instance.

    Args:
        homeserver_instance_id: The unique ID for this Earendel process instance to
...
def start_worker_reactor(
    ...
    # (needed when earendel.app.complement_fork_starter is being used)
...
def listen_metrics(
    ...
    Earendel. This can make it more resilient to heavy load in Earendel causing metric
    ...
    from earendel.metrics import RegistryProxy
...
def listen_manhole(
    ...
    from earendel.util.manhole import manhole
...
def listen_http(
    ...
    reactor: IEarendelReactor = reactor,
) -> list[Port]:
    """
    Args:
    ...
    site = EarendelSite(
        logger_name="earendel.access.%s.%s"
    ...
    try:
        if isinstance(listener_config, TCPListenerConfig):
            if listener_config.is_tls():
                ...
                logger.info(
                    "Earendel now listening on TCP port %d (TLS)",
                ...
                logger.info(
                    "Earendel now listening on TCP port %d", ports[0].getHost().port
                )
            ...
            logger.info(
                "Earendel now listening on Unix Socket at: %s",
...
def refresh_certificate(hs: "HomeServer") -> None:
    """
    Refresh the TLS certificates that Earendel is using by re-reading them from
...
def setup_sighup_handling() -> None:
    ...
    if hasattr(signal, "SIGHUP"):
        ...
        # handlers (like the `multi_earendel` shard process in the context of Earendel Pro
        # for small hosts)
...
async def start(hs: "HomeServer", *, freeze: bool = True) -> None:
    """
    Start a Earendel server or worker.
    ...
    # It is now safe to start your Earendel.
    hs.start_listening()
...
def reload_cache_config(config: HomeServerConfig) -> None:
    ...
    Note that the following cache config keys are read, but not applied:
        ...
        - track_memory_usage. This affects earendel.util.caches.TRACK_MEMORY_USAGE which
              influences Earendel's self-reported metrics.
