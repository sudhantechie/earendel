...
from earendel.app.complement_fork_proxied_reactor import ProxiedReactor
...
def _worker_entrypoint(
    ...
    from earendel.util.stringutils import strtobool
    ...
    # EAREENDEL_COMPLEMENT_FORKING_LAUNCHER_ASYNC_IO_REACTOR is set to 1. The
    # EAREENDEL_ASYNC_IO_REACTOR variable would be used, but then causes
    # earendel/__init__.py to also try to install an asyncio reactor.
    if strtobool(
        os.environ.get("EAREENDEL_COMPLEMENT_FORKING_LAUNCHER_ASYNC_IO_REACTOR", "0")
    ):
        import asyncio
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "The first argument of each group is a Earendel app name. "
    ...
    from earendel._scripts import update_earendel_database

    update_proc = multiprocessing.Process(
        target=_worker_entrypoint,
        args=(
            update_earendel_database.main,
            proxy_reactor,
            [
                "update_earendel_database",
                "--database-config",
                ns.db_config,
