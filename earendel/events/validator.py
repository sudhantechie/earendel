...
from earendel.api.room_versions import EventFormatVersions
...
from earendel.storage.controllers.state import server_acl_evaluator_from_event
from earendel.types import EventID, JsonDict, JsonMapping, RoomID, StrCollection, UserID
...
class EventValidator:
    def validate_new(self, event: EventBase, config: HomeServerConfig) -> None:
        ...
        for k in required:
            if k not in event:
                raise EarendelError(400, "Event does not have key %s" % (k,))
    ...
    def _validate_retention(self, event: EventBase) -> None:
        ...
        if not event.is_state():
            raise EarendelError(code=400, msg="must be a state event")
    ...
    def validate_builder(self, event: EventBase | EventBuilder) -> None:
        ...
        for s in strings:
            if not isinstance(getattr(event, s), str):
                raise EarendelError(400, "Not '%s' a string type" % (s,))
        ...
        if not create_event_as_room_id:
            ...
            if event.room_version.msc4291_room_ids_as_hashes and not RoomID.is_valid(
                ...
                raise EarendelError(400, f"Invalid room ID '{event.room_id}'")
        ...
        if event.type == EventTypes.Message:
            ...
            if "membership" not in event.content:
                raise EarendelError(400, "Content has not membership key")
            ...
            if "replacement_room" not in event.content:
                raise EarendelError(400, "Content has no replacement_room key")
    ...
    def _ensure_strings(self, d: JsonMapping, keys: StrCollection) -> None:
        for s in keys:
            if s not in d:
                raise EarendelError(400, "'%s' not in content" % (s,))
            if not isinstance(d[s], str):
                raise EarendelError(400, "'%s' not a string type" % (s,))

    def _ensure_state_event(self, event: EventBase | EventBuilder) -> None:
        if not event.is_state():
            raise EarendelError(400, "'%s' must be state events" % (event.type,))
...
def _validation_error_to_api_error(err: jsonschema.ValidationError) -> EarendelError:
    """
    Converts a JSONSchema `ValidationError` to a `EarendelError` that can be thrown
