...
from earendel.api.constants import EventTypes
from earendel.api.room_versions import KNOWN_ROOM_VERSIONS, StateResolutionVersions
...
from earendel.logging.context import ContextResourceUsage
from earendel.logging.opentracing import tag_args, trace
from earendel.metrics import SERVER_NAME_LABEL
from earendel.replication.http.state import ReplicationUpdateCurrentStateRestServlet
from earendel.state import v1, v2
from earendel.storage.databases.main.event_federation import StateDifference
from earendel.storage.databases.main.events_worker import EventRedactBehaviour
from earendel.types import StateMap, StrCollection
...
from earendel.util.metrics import Measure, measure_func
from earendel.util.stringutils import shortstr

if TYPE_CHECKING:
    from earendel.server import HomeServer
    from earendel.storage.controllers import StateStorageController
    from earendel.storage.databases.main import DataStore
    from earendel.storage.databases.state.deletion import StateDeletionDataStore

logger = logging.getLogger(__name__)
metrics_logger = logging.getLogger("earendel.state.metrics")

# Metrics for number of state groups involved in a resolution.
state_groups_histogram = Histogram(
    "earendel_state_number_state_groups_in_resolution",
...
class StateHandler:
    ...
    async def update_current_state(self, room_id: str) -> None:
        ...
        Raises:
            EarendelError(502): if all attempts to connect to the event persister worker
                fail
        """
...
_biggest_room_by_cpu_counter = Counter(
    "earendel_state_res_cpu_for_biggest_room_seconds",
...
_biggest_room_by_db_counter = Counter(
    "earendel_state_res_db_for_biggest_room_seconds",
...
_cpu_times = Histogram(
    "earendel_state_res_cpu_for_all_rooms_seconds",
    "CPU time (utime+stime) spent computing a single state resolution",
    labelnames=[SERVER_NAME_LABEL],
)
_db_times = Histogram(
    "earendel_state_res_db_for_all_rooms_seconds",
