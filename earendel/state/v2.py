...
from earendel.api.constants import CREATOR_POWER_LEVEL, EventTypes
from earendel.api.errors import AuthError
from earendel.api.room_versions import RoomVersion, StateResolutionVersions
...
from earendel.types import MutableStateMap, StateMap, StrCollection
...
class Clock(Protocol):
    # This is usually earendel.util.Clock, but it's replaced with a FakeClock in tests.
...
class StateResolutionStore(Protocol):
    # This is usually earendel.state.StateResolutionStore, but it's replaced with a
...
async def _iterative_auth_checks(
    ...
    for idx, event_id in enumerate(event_ids, start=1):
        ...
        if event.rejected_reason is not None:
            ...
            #       state resolution should be accepted. Earendel does not handle the
            ...
            #       https://github.com/sudhantechie/earendel/issues/13797
            continue
