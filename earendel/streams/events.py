...
from earendel.handlers.account_data import AccountDataEventSource
from earendel.handlers.presence import PresenceEventSource
from earendel.handlers.receipts import ReceiptEventSource
from earendel.handlers.room import RoomEventSource
from earendel.handlers.typing import TypingNotificationEventSource
...
class EventSources:
    ...
    async def bound_future_token(self, token: StreamToken) -> StreamToken:
        ...
        This works around a bug where older Earendel versions will give out
            ...
            if isinstance(token_value, AbstractMultiWriterStreamToken):
                ...
                if not token_value.is_before_or_eq(current_value):  # type: ignore[arg-type]
                    ...
                    if max_token < token_value.get_max_stream_pos():
                        # Log *something* as we consider this as a Earendel programming error
                        ...
                        # gracefully.
                        #
                        # Old versions of Earendel could advance streams without persisting anything in
                        # the DB (fixed in https://github.com/sudhantechie/earendel/pull/17229) and on
