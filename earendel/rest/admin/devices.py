...
class DeviceRestServlet(RestServlet):
    ...
    async def on_GET(
        self, request: EarendelRequest, user_id: str, device_id: str
    ) -> tuple[int, JsonDict]:
        await assert_requester_is_admin(self.auth, request)

        target_user = UserID.from_string(user_id)
        if not self.is_mine(target_user):
            raise EarendelError(HTTPStatus.BAD_REQUEST, "Can only lookup local users")
...
class DevicesRestServlet(RestServlet):
    ...
    async def on_POST(
        ...
        if not device_id:
            raise EarendelError(HTTPStatus.BAD_REQUEST, "Missing device_id")
        if not isinstance(device_id, str):
            raise EarendelError(HTTPStatus.BAD_REQUEST, "device_id must be a string")
