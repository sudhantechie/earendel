...
from earendel.api.constants import Direction, ProfileFields
from earendel.api.errors import Codes, NotFoundError, EarendelError
...
from earendel.rest.admin._base import (
    admin_patterns,
    assert_requester_is_admin,
    assert_user_is_admin,
)
from earendel.rest.client._base import client_patterns
from earendel.storage.databases.main import UserPaginateResponse
from earendel.storage.databases.main.registration import ExternalIDReuseException
from earendel.storage.databases.main.stats import UserSortOrder
from earendel.types import JsonDict, JsonMapping, TaskStatus, UserID
...
user_registration_sensitive_debug_logger = logging.getLogger(
    "earendel.rest.admin.users.registration_debug"
)
"""
...
in the logs. Requires explicitly setting `earendel.rest.admin.users.registration_debug`
...
class UsersRestServletV2(RestServlet):
    ...
    This needs user to have administrator access in Earendel.
    ...
    def _parse_parameter_deactivated(self, request: EarendelRequest) -> bool | None:
        """
...
class UserRestServletV2Get(RestServlet):
    ...
    async def on_GET(
        self, request: EarendelRequest, user_id: str
    ) -> tuple[int, JsonMapping]:
        await assert_requester_is_admin(self.auth, request)

        target_user = UserID.from_string(user_id)
        if not self.hs.is_mine(target_user):
            raise EarendelError(HTTPStatus.BAD_REQUEST, "Can only look up local users")
...
class UserRestServletV2(UserRestServletV2Get):
    ...
    async def on_PUT(
        ...
        if user_type is not None and user_type not in self._all_user_types:
            raise EarendelError(HTTPStatus.BAD_REQUEST, "Invalid user type")
        ...
        if password is not None:
            if not isinstance(password, str) or len(password) > 512:
                raise EarendelError(HTTPStatus.BAD_REQUEST, "Invalid password")
            ...
            if threepids is not None:
                ...
                for medium, address in del_threepids:
                    try:
                        ...
                        raise EarendelError(500, "Failed to remove threepids")
...
class UserRegisterServlet(RestServlet):
    ...
    def on_GET(self, request: EarendelRequest) -> tuple[int, JsonDict]:
        """
    ...
    async def on_POST(self, request: EarendelRequest) -> tuple[int, JsonDict]:
        ...
        if nonce not in self.nonces:
            raise EarendelError(HTTPStatus.BAD_REQUEST, "unrecognised nonce")
        ...
        if "username" not in body:
            ...
            if not isinstance(body["username"], str) or len(body["username"]) > 512:
                raise EarendelError(HTTPStatus.BAD_REQUEST, "Invalid username")
        ...
        if not hmac.compare_digest(want_mac.encode("ascii"), got_mac.encode("ascii")):
            ...
            if user_registration_sensitive_debug_logger.isEnabledFor(logging.DEBUG):
                ...
                logger.debug(
                    (
                        ...
                        "If you need more information, explicitly enable the `earendel.rest.admin.users.registration_debug` "
            ...
            raise EarendelError(HTTPStatus.FORBIDDEN, "HMAC incorrect")
        ...
        from earendel.rest.client.register import RegisterRestServlet
...
class WhoisRestServlet(RestServlet):
    ...
    async def on_GET(
        ...
        if not self.is_mine(target_user):
            raise EarendelError(HTTPStatus.BAD_REQUEST, "Can only whois a local user")
...
class DeactivateAccountRestServlet(RestServlet):
    ...
    async def on_POST(
        self, request: EarendelRequest, target_user_id: str
    ) -> tuple[int, JsonDict]:
        requester = await self.auth.get_user_by_req(request)
        await assert_user_is_admin(self.auth, requester)
...
class SuspendAccountRestServlet(RestServlet):
    ...
    async def on_PUT(
        ...
        if not self.is_mine(UserID.from_string(target_user_id)):
            raise EarendelError(HTTPStatus.BAD_REQUEST, "Can only suspend local users")
...
class SearchUsersRestServlet(RestServlet):
    ...
    async def on_GET(
        ...
        """Get request to search user table for specific users according to
        search term.
        This needs user to have a administrator access in Earendel.
        """
        await assert_requester_is_admin(self.auth, request)
        ...
        if not self.is_mine(target_user):
            raise EarendelError(HTTPStatus.BAD_REQUEST, "Can only users a local user")
...
class UserAdminServlet(RestServlet):
    ...
    async def on_PUT(
        ...
        if target_user == auth_user and not set_admin_to:
            raise EarendelError(HTTPStatus.BAD_REQUEST, "You may not demote yourself.")
...
class RedactUserStatus(RestServlet):
    ...
    async def on_GET(
        self, request: EarendelRequest, redact_id: str
    ) -> tuple[int, JsonDict]:
        await assert_requester_is_admin(self._auth, request)
