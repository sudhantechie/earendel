...
from earendel.api.constants import Direction, EventTypes, JoinRules, Membership
...
from earendel.handlers.pagination import (
...
from earendel.rest.client.room import SerializeMessagesDeps, encode_messages_response
from earendel.storage.databases.main.room import RoomSortOrder
...
if TYPE_CHECKING:
    ...
    from earendel.handlers.room import RoomShutdownHandler
...
class DeleteRoomStatusByDeleteIdRestServlet(RestServlet):
    ...
    async def on_GET(
        self, request: EarendelRequest, delete_id: str
    ) -> tuple[int, JsonDict]:
        await assert_requester_is_admin(self._auth, request)
...
class MakeRoomAdminRestServlet(ResolveRoomIdMixin, RestServlet):
    ...
    async def on_POST(
        ...
        if not filtered_room_state:
            raise EarendelError(HTTPStatus.BAD_REQUEST, "Server not in room")
