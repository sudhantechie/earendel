...
from earendel.api.constants import Direction
...
class QueryMediaById(RestServlet):
    ...
    async def on_GET(
        self, request: EarendelRequest, server_name: str, media_id: str
    ) -> tuple[int, JsonDict]:
    ...
    async def on_GET(self, request: EarendelRequest) -> tuple[int, JsonDict]:
        ...
        if from_id > max_id:
            ...
            raise EarendelError(
                ...
                "This indicates either a Earendel programming error (as we should never hand out "
        ...
        if not await self.notifier.wait_for_stream_token(from_token):
            raise EarendelError(
                ...
                "with Earendel or the worker deployment lagging behind the replication stream. Please try "
...
class ProtectMediaByID(RestServlet):
    ...
    async def on_POST(
        self, request: EarendelRequest, media_id: str
    ) -> tuple[int, JsonDict]:
        await assert_requester_is_admin(self.auth, request)
...
class DeleteMediaByID(RestServlet):
    ...
    async def on_DELETE(
        ...
        if not self._is_mine_server_name(server_name):
            raise EarendelError(HTTPStatus.BAD_REQUEST, "Can only delete local media")
...
class DeleteMediaByDateSize(RestServlet):
    ...
    async def on_POST(
        self, request: EarendelRequest, server_name: str | None = None
    ) -> tuple[int, JsonDict]:
        await assert_requester_is_admin(self.auth, request)
