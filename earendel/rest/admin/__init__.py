...
from earendel.handlers.pagination import PURGE_HISTORY_ACTION_NAME
...
from earendel.rest.admin._base import admin_patterns, assert_requester_is_admin
from earendel.rest.admin.background_updates import (
    BackgroundUpdateEnabledRestServlet,
    BackgroundUpdateRestServlet,
    BackgroundUpdateStartJobRestServlet,
)
from earendel.rest.admin.devices import (
    DeleteDevicesRestServlet,
    DeviceRestServlet,
    DevicesRestServlet,
)
from earendel.rest.admin.event_reports import (
    EventReportDetailRestServlet,
    EventReportsRestServlet,
)
from earendel.rest.admin.events import (
    EventRestServlet,
)
from earendel.rest.admin.experimental_features import ExperimentalFeaturesRestServlet
from earendel.rest.admin.federation import (
    DestinationMembershipRestServlet,
    DestinationResetConnectionRestServlet,
    DestinationRestServlet,
    ListDestinationsRestServlet,
)
from earendel.rest.admin.media import ListMediaInRoom, register_servlets_for_media_repo
from earendel.rest.admin.registration_tokens import (
    ListRegistrationTokensRestServlet,
    NewRegistrationTokenRestServlet,
    RegistrationTokenRestServlet,
)
from earendel.rest.admin.rooms import (
    AdminRoomHierarchy,
...
from earendel.rest.admin.scheduled_tasks import ScheduledTasksRestServlet
from earendel.rest.admin.server_notice_servlet import SendServerNoticeServlet
from earendel.rest.admin.statistics import (
    LargestRoomsStatistics,
    UserMediaStatisticsRestServlet,
)
from earendel.rest.admin.user_reports import (
    UserReportDetailRestServlet,
    UserReportsRestServlet,
)
from earendel.rest.admin.username_available import UsernameAvailableRestServlet
from earendel.rest.admin.users import (
...
from earendel.types import JsonDict, RoomStreamToken, TaskStatus
...
class VersionServlet(RestServlet):
    ...
    def __init__(self, hs: "HomeServer"):
        self.res = {"server_version": EAREENDEL_VERSION}
...
class PurgeHistoryRestServlet(RestServlet):
    ...
    async def on_POST(
        self, request: EarendelRequest, room_id: str, event_id: str | None
    ) -> tuple[int, JsonDict]:
        ...
        if event_id is not None:
            ...
            if event.room_id != room_id:
                raise EarendelError(HTTPStatus.BAD_REQUEST, "Event is for wrong room.")
...
class PurgeHistoryStatusRestServlet(RestServlet):
    ...
    async def on_GET(
        self, request: EarendelRequest, purge_id: str
    ) -> tuple[int, JsonDict]:
        await assert_requester_is_admin(self.auth, request)
