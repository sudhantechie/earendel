...
from earendel.api.errors import Codes, LoginError, EarendelError
from earendel.api.urls import CLIENT_API_PREFIX
from earendel.http.server import HttpServer, respond_with_html, respond_with_redirect
from earendel.http.servlet import RestServlet, parse_string
...
class AuthRestServlet(RestServlet):
    ...
    async def on_GET(self, request: EarendelRequest, stagetype: str) -> None:
        session = parse_string(request, "session")
        if not session:
            raise EarendelError(400, "No session supplied")
        ...
        if stagetype == LoginType.RECAPTCHA:
            ...
            raise EarendelError(404, "Unknown auth stage type")
    ...
    async def on_POST(self, request: Request, stagetype: str) -> None:
        ...
        if stagetype == LoginType.RECAPTCHA:
            ...
            if not response:
                raise EarendelError(400, "No captcha response supplied")
            ...
            raise EarendelError(404, "Fallback SSO auth does not support POST requests.")
