...
from earendel.api.errors import NotFoundError, EarendelError, UnrecognizedRequestError
...
from earendel.rest.client._base import client_patterns, interactive_auth_handler
from earendel.types import JsonDict
from earendel.types.rest import RequestBodyModel
from earendel.types.rest.client import AuthenticationData
...
class DeleteDevicesRestServlet(RestServlet):
    ...
    async def on_POST(self, request: EarendelRequest) -> tuple[int, JsonDict]:
        ...
        except errors.EarendelError as e:
...
class DeviceRestServlet(RestServlet):
    ...
    async def on_GET(
        self, request: EarendelRequest, device_id: str
    ) -> tuple[int, JsonDict]:
        requester = await self.auth.get_user_by_req(request, allow_guest=True)
...
class DehydratedDeviceV2Servlet(RestServlet):
    ...
    async def on_DELETE(self, request: EarendelRequest) -> tuple[int, JsonDict]:
        requester = await self.auth.get_user_by_req(request)
    ...
    async def on_PUT(self, request: EarendelRequest) -> tuple[int, JsonDict]:
