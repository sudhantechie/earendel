...
from earendel.handlers.profile import MAX_CUSTOM_FIELD_LEN
...
from earendel.util.stringutils import is_namedspaced_grammar

if TYPE_CHECKING:
    from earendel.server import HomeServer


def _read_propagate(hs: "HomeServer", request: EarendelRequest) -> bool:
...
class ProfileFieldRestServlet(RestServlet):
    ...
    async def on_GET(
        self, request: EarendelRequest, user_id: str, field_name: str
    ) -> tuple[int, JsonDict]:
        requester_user = None
        ...
        if not field_name:
            raise EarendelError(400, "Field name too short", errcode=Codes.INVALID_PARAM)

        if len(field_name.encode("utf-8")) > MAX_CUSTOM_FIELD_LEN:
            raise EarendelError(400, "Field name too long", errcode=Codes.KEY_TOO_LARGE)
