...
from earendel.types import EventID, JsonDict, RoomID
...
class ReceiptRestServlet(RestServlet):
    ...
    async def on_POST(
        self, request: EarendelRequest, room_id: str, receipt_type: str, event_id: str
    ) -> tuple[int, JsonDict]:
        requester = await self.auth.get_user_by_req(request)

        if not RoomID.is_valid(room_id) or not event_id.startswith(EventID.SIGIL):
            raise EarendelError(400, "A valid room ID and event ID must be specified")
