...
import earendel
import earendel.api.auth
import earendel.types
...
from earendel.config.ratelimiting import FederationRatelimitSettings
from earendel.config.server import is_threepid_reserved
from earendel.handlers.auth import AuthHandler
from earendel.handlers.ui_auth import UIAuthSessionDataConstants
from earendel.http.server import HttpServer, finish_request, respond_with_html
...
from earendel.metrics import SERVER_NAME_LABEL, threepid_send_requests
from earendel.push.mailer import Mailer
...
from earendel.util.stringutils import assert_valid_client_secret, random_string
from earendel.util.threepids import (
    canonicalise_email,
    check_3pid_allowed,
    validate_email,
)
...
class EmailRegisterRequestTokenRestServlet(RestServlet):
    ...
    async def on_POST(self, request: EarendelRequest) -> tuple[int, JsonDict]:
        ...
        # in earendel/rest/client/account.py)
        try:
        ...
        if existing_user_id is not None:
            ...
            raise EarendelError(400, "Email is already in use", Codes.THREEPID_IN_USE)

        # Send registration emails from Earendel
...
class RegisterRestServlet(RestServlet):
    ...
    async def on_POST(self, request: EarendelRequest) -> tuple[int, JsonDict]:
        ...
        if not isinstance(client_requested_refresh_tokens, bool):
            raise EarendelError(400, "`refresh_token` should be true or false.")
        ...
        if "username" in body:
            ...
            if not isinstance(desired_username, str) or len(desired_username) > 512:
                raise EarendelError(400, "Invalid username")
        ...
        if body.get("type") == APP_SERVICE_REGISTRATION_TYPE:
            ...
            if desired_username is None:
                raise EarendelError(400, "Desired Username is missing or not a string")
        ...
        if not self._registration_enabled:
            raise EarendelError(403, "Registration has been disabled", Codes.FORBIDDEN)
        ...
        if password is not None:
            if not isinstance(password, str) or len(password) > 512:
                raise EarendelError(400, "Invalid password")
        ...
        if registered_user_id is not None:
            ...
            if not password_hash:
                raise EarendelError(400, "Missing params: password", Codes.MISSING_PARAM)
    ...
    async def _do_guest_registration(
        ...
        if not self.hs.config.registration.allow_guest_access:
            raise EarendelError(403, "Guest access is disabled")
        ...
        device_id = earendel.api.auth.GUEST_DEVICE_ID
        initial_display_name = params.get("initial_device_display_name")
        (
            device_id,
            access_token,
