...
from earendel.api.constants import ApprovalNoticeMedium
...
from earendel.api.urls import CLIENT_API_PREFIX
from earendel.appservice import ApplicationService
from earendel.handlers.sso import SsoIdentityProvider, SsoSetupError
from earendel.http import get_request_uri
from earendel.http.server import HttpServer, finish_request
...
from earendel.http.site import RequestInfo, EarendelRequest
from earendel.rest.client._base import client_patterns
from earendel.rest.well_known import WellKnownBuilder
from earendel.types import JsonDict, UserID
...
class LoginRestServlet(RestServlet):
    ...
    def on_GET(self, request: EarendelRequest) -> tuple[int, JsonDict]:
        ...
        # earendel currently only gives out these tokens as part of the
    ...
    async def on_POST(self, request: EarendelRequest) -> tuple[int, LoginResponse]:
        ...
        try:
            ...
            raise EarendelError(400, "Missing JSON keys.")
    ...
    async def _do_appservice_login(
        ...
        *,
        ...
        if not isinstance(user, str):
            raise EarendelError(400, "Invalid user in identifier", Codes.INVALID_PARAM)
        ...
        return await self._complete_login(
            ...
            # is not actually created in Earendel.
...
class RefreshTokenServlet(RestServlet):
    ...
    async def on_POST(self, request: EarendelRequest) -> tuple[int, JsonDict]:
        ...
        if not isinstance(token, str):
            raise EarendelError(400, "Invalid param: refresh_token", Codes.INVALID_PARAM)
...
class SsoRedirectServlet(RestServlet):
    ...
    async def on_GET(self, request: EarendelRequest, idp_id: str | None = None) -> None:
        if not self._public_baseurl:
            raise EarendelError(400, "SSO requires a valid public_baseurl")
...
class CasTicketServlet(RestServlet):
    ...
    async def on_GET(self, request: EarendelRequest) -> None:
        ...
        if not client_redirect_url and not session:
            ...
            raise EarendelError(400, message, errcode=Codes.MISSING_PARAM)
