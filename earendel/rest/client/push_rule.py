...
from earendel.handlers.push_rules import InvalidRuleException, RuleSpec, check_actions
...
from earendel.push.rulekinds import PRIORITY_CLASS_MAP
...
class PushRuleRestServlet(RestServlet):
    ...
    async def on_PUT(self, request: EarendelRequest, path: str) -> tuple[int, JsonDict]:
    ...
    async def handle_put(
        self, request: EarendelRequest, path: str, user_id: str
    ) -> tuple[int, JsonDict]:
        ...
        if "/" in spec.rule_id or "\\" in spec.rule_id:
            raise EarendelError(400, "rule_id may not contain slashes")
        ...
        if spec.attr:
            try:
                ...
                raise EarendelError(400, "Invalid actions: %s" % e)
        ...
        if spec.rule_id.startswith("."):
            ...
            raise EarendelError(400, "cannot add new rule_ids that start with '.'")
    ...
    async def on_DELETE(
        self, request: EarendelRequest, path: str
    ) -> tuple[int, JsonDict]:
        if not self._is_push_worker:
    ...
    async def on_GET(self, request: EarendelRequest, path: str) -> tuple[int, JsonDict]:
        requester = await self.auth.get_user_by_req(request)
