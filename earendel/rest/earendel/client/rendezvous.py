...
from earendel.api.errors import UnrecognizedRequestError
...
class MSC4108RendezvousSessionResource(DirectServeJsonResource):
    ...
    def _async_render_PUT(self, request: EarendelRequest) -> None:
        postpath: list[bytes] = request.postpath  # type: ignore
        if len(postpath) != 1:
            raise UnrecognizedRequestError()
        session_id = postpath[0].decode("ascii")
    ...
    def _async_render_DELETE(self, request: EarendelRequest) -> None:
        postpath: list[bytes] = request.postpath  # type: ignore
        if len(postpath) != 1:
            raise UnrecognizedRequestError()
        session_id = postpath[0].decode("ascii")
