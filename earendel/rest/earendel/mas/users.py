...
from earendel.types import JsonDict, UserID, UserInfo, create_requester
...
class MasProvisionUserResource(MasBaseResource):
    ...
    class PostBody(RequestBodyModel):
        ...
        but not in Earendel, the token introspection response will prevent them from using
        their account.

        However, having a local copy of the locked state in Earendel is useful for excluding
...
class MasIsLocalpartAvailableResource(MasBaseResource):
    ...
    async def _async_render_GET(
        ...
        if localpart is None:
            raise EarendelError(400, "Missing localpart")
