...
from earendel.config import find_config_files

MAIN_PROCESS = "earendel.app.homeserver"
...
Setting 'earendelctl_cache_factor' in the config is deprecated. Instead, please do
one of the following:
 - Either set the environment variable 'EAREENDEL_CACHE_FACTOR'
...
def start(pidfile: str, app: str, config_files: Iterable[str], daemonize: bool) -> bool:
    """Attempts to start a earendel main or worker process.
    Args:
        pidfile: the pidfile we expect the process to create
        app: the python module to run
        config_files: config files to pass to earendel
        daemonize: if True, will include a --daemonize argument to earendel
    ...
    try:
        ...
        err = "%s(%s) failed to start (exit code: %d). Check the Earendel logfile" % (
            app,
            ",".join(config_files),
            e.returncode,
        )
        if daemonize:
            err += ", or run earendelctl with --no-daemonize"
        err += "."
...
def stop(pidfile: str, app: str) -> int | None:
    """Attempts to kill a earendel worker from the pidfile.
    Args:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="whether to start, stop or restart the earendel",
    )
    parser.add_argument(
        "configfile",
        nargs="?",
    ...
    parser.add_argument(
        ...
        " and the main earendel process",
    )
    parser.add_argument(
        "--no-daemonize",
        action="store_false",
        dest="daemonize",
        help="Run earendel in the foreground for debugging. "
    ...
    cache_factor = config.get("earendelctl_cache_factor")
    start_stop_earendel = True

    if cache_factor:
        write(SYNCTL_CACHE_FACTOR_WARNING)
        os.environ["EAREENDEL_CACHE_FACTOR"] = str(cache_factor)

    cache_factors = config.get("earendelctl_cache_factors", {})
    for cache_name, factor in cache_factors.items():
        os.environ["EAREENDEL_CACHE_FACTOR_" + cache_name.upper()] = str(factor)

    worker_configfiles = []
    if options.worker:
        start_stop_earendel = False
    ...
    if options.all_processes:
        # To start the main earendel with -a you need to add a worker file
        # with worker_app == "earendel.app.homeserver"
        start_stop_earendel = False
    ...
    for worker_configfile in worker_configfiles:
        ...
        if worker_app == "earendel.app.homeserver":
            # We need to special case all of this to pick up options that may
            # be set in the main config file or in this worker config file.
            worker_pidfile = worker_config.get("pid_file") or pidfile
            worker_cache_factor = (
                worker_config.get("earendelctl_cache_factor") or cache_factor
            )
            worker_cache_factors = (
                worker_config.get("earendelctl_cache_factors") or cache_factors
            )
            ...
            worker_cache_factor = worker_config.get("earendelctl_cache_factor")
            worker_cache_factors = worker_config.get("earendelctl_cache_factors", {})
        workers.append(
            Worker(
                worker_app,
                worker_configfile,
    ...
    if action == "start" or action == "restart":
        ...
        for worker in workers:
            ...
            if worker.cache_factor:
                os.environ["EAREENDEL_CACHE_FACTOR"] = str(worker.cache_factor)
