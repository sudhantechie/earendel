...
from earendel.federation.transport.server import (
...
from earendel.rest import ClientRestResource
from earendel.rest.key.v2 import RemoteKey
...
class EndpointDescription:
    ...
    #  - what 'opinionated earendel worker class' (event_creator, synchrotron, etc) does
    #    it go in?
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Updates a earendel database to the latest schema and optionally runs background updates"
            " on it."
        )
    )
    ...
    parser.add_argument(
        ...
        required=True,
        help="Earendel configuration file",
    )
    ...
    worker_paths = get_registered_paths_for_default(
        "earendel.app.generic_worker", config
    )
