...
import earendel
from earendel.api.errors import Codes
from earendel.logging.opentracing import trace
from earendel.media._base import FileInfo
from earendel.media.media_storage import ReadableFileWrapper
from earendel.spam_checker_api import RegistrationBehaviour
from earendel.types import JsonDict, RoomAlias, UserProfile
...
def load_legacy_spam_checkers(hs: "earendel.server.HomeServer") -> None:
...
class SpamCheckerModuleApiCallbacks:
    ...
    def __init__(self, hs: "earendel.server.HomeServer") -> None:
    ...
    async def check_event_for_spam(
        self, event: "earendel.events.EventBase"
        ...
        for callback in self._check_event_for_spam_callbacks:
            with Measure(
                ...
                if res is False or res == self.NOT_SPAM:
                    ...
                    # return value `True`
                    return earendel.api.errors.Codes.FORBIDDEN, {}
                elif (
                    isinstance(res, tuple)
                    and len(res) == 2
                    and isinstance(res[0], earendel.api.errors.Codes)
                    and isinstance(res[1], dict)
                ):
                    return res
                elif isinstance(res, earendel.api.errors.Codes):
                    return res, {}
    ...
    async def user_may_send_3pid_invite(
        ...
        Note that if the threepid is already associated with a Matrix user ID, Earendel
        ...
        should do::
            ...
            async def check_media_file_for_spam(
                ...
                if buffer.getvalue() == b"Hello World":
                    return earendel.module_api.NOT_SPAM
