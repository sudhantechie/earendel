...
from earendel.api import errors
from earendel.api.constants import ProfileFields
from earendel.api.errors import EarendelError
from earendel.api.presence import UserPresenceState
from earendel.config import ConfigError
from earendel.config.repository import MediaUploadLimit
from earendel.events import EventBase
from earendel.events.presence_router import (
    GET_INTERESTED_USERS_CALLBACK,
    GET_USERS_FOR_STATES_CALLBACK,
    PresenceRouter,
)
from earendel.events.utils import ADD_EXTRA_FIELDS_TO_UNSIGNED_CLIENT_EVENT_CALLBACK
from earendel.handlers.account_data import ON_ACCOUNT_DATA_UPDATED_CALLBACK
from earendel.handlers.auth import (
...
from earendel.handlers.push_rules import RuleSpec, check_actions
from earendel.http.client import SimpleHttpClient
from earendel.http.server import (
    DirectServeHtmlResource,
    DirectServeJsonResource,
    respond_with_html,
)
from earendel.http.servlet import parse_json_object_from_request
from earendel.http.site import EarendelRequest
from earendel.logging.context import (
    defer_to_thread,
    defer_to_threadpool,
    make_deferred_yieldable,
    run_in_background,
)
from earendel.metrics.background_process_metrics import (
    run_as_background_process as _run_as_background_process,
)
from earendel.module_api.callbacks.account_validity_callbacks import (
...
from earendel.module_api.callbacks.federation import (
    ON_EVENT_DELIVERED_OVER_FEDERATION_CALLBACK,
)
from earendel.module_api.callbacks.media_repository_callbacks import (
...
from earendel.module_api.callbacks.ratelimit_callbacks import (
    GET_RATELIMIT_OVERRIDE_FOR_USER_CALLBACK,
    RatelimitOverride,
)
from earendel.module_api.callbacks.spamchecker_callbacks import (
...
from earendel.module_api.callbacks.third_party_event_rules_callbacks import (
...
from earendel.push.httppusher import HttpPusher
from earendel.rest.client.login import LoginResponse
from earendel.storage import DataStore
from earendel.storage.background_updates import (
    DEFAULT_BATCH_SIZE_CALLBACK,
    MIN_BATCH_SIZE_CALLBACK,
    ON_UPDATE_CALLBACK,
)
from earendel.storage.database import DatabasePool, LoggingTransaction
from earendel.storage.databases.main.roommember import ProfileInfo
from earendel.types import (
    Absent,
    AbsentType,
...
from earendel.types.state import StateFilter
from earendel.util.async_helpers import maybe_awaitable
from earendel.util.caches.descriptors import CachedFunction, cached as _cached
from earendel.util.clock import Clock
from earendel.util.duration import Duration
from earendel.util.frozenutils import freeze
...
def run_as_background_process(
    ...
    **kwargs: Any,
    ...
    # `run_as_background_process` internally in Earendel requires `server_name` now, we
    # just have to stub this out with a placeholder value and tell people to use the new
    # function instead.
    stub_server_name = "earendel_module_running_from_unknown_server"
...
class ModuleApi:
    ...
    def http_client(self) -> SimpleHttpClient:
        ...
        An instance of earendel.http.client.SimpleHttpClient

        Added in Earendel v1.22.0.
        """
    ...
    def public_room_list_manager(self) -> "PublicRoomListManager":
        ...
        An instance of earendel.module_api.PublicRoomListManager
    ...
    def register_user(
        ...
        Added in Earendel v1.2.0.
        Changed in Earendel v1.56.0: add 'admin' argument to register the user as admin.
        ...
        Raises:
            EarendelError if there is an error performing the registration. Check the
    ...
    def invalidate_access_token(
        ...
        Raises:
            earendel.api.errors.AuthError: the access token is invalid
        """
    ...
    async def update_room_membership(
        ...
        Added in Earendel v1.46.0.
        Changed in Earendel v1.65.0: Added the 'remote_room_hosts' parameter.
        ...
        Raises:
            ...
            EarendelError if the module attempts to send a membership event that isn't
    ...
    async def create_and_send_event_into_room(self, event_dict: JsonDict) -> EventBase:
        ...
        Raises:
            EarendelError if the event was not allowed.
        """
    ...
    async def set_presence_for_users(
        ...
        from earendel.handlers.presence import PresenceHandler
        ...
        **kwargs: object,
        ...
        Args:
            f: The function to call repeatedly. f can be either synchronous or
                asynchronous, and must follow Earendel's logcontext rules.
                More info about logcontexts is available at
                https://github.com/sudhantechie/earendel/latest/log_contexts.html
    ...
    def delayed_background_call(
        ...
        **kwargs: object,
        ...
        The scheduled call is not persistent: if the current Earendel instance is
        restarted before the call is made, the call will not be made.

        Added in Earendel v1.90.0.
    ...
    def read_templates(
        ...
        By default, Earendel will look for these templates in its configured template
    ...
    async def defer_to_thread(
        ...
        **kwargs: P.kwargs,
        ...
        """Runs the given function in a separate thread from Earendel's thread pool.
    ...
    async def defer_to_threadpool(
        ...
        **kwargs: P.kwargs,
        ...
        Allows specifying a custom thread pool instead of using the default Earendel
        one. To use the default Earendel threadpool, use `defer_to_thread` instead.

        Added in Earendel v1.140.0.
    ...
    async def check_username(self, username: str) -> None:
        ...
        Raises:
            EarendelError with the errcode "M_USER_IN_USE" if the username is already in
            use.
        """
    ...
    async def store_remote_3pid_association(
        ...
        Args:
            ...
            id_server: The identity server the 3PID association has been registered on.
                ...
                identity server using HTTPS (unless specified otherwise by Earendel's
    ...
    def check_push_rule_actions(self, actions: list[str | dict[str, str]]) -> None:
        ...
        Added in Earendel v1.58.0.

        Args:
            actions: the actions to check.

        Raises:
            earendel.module_api.errors.InvalidRuleException if the actions are invalid.
        """
    ...
    async def set_push_rule_action(
        ...
        Raises:
            ...
            earendel.module_api.errors.RuleNotFoundException if the rule being modified
                can't be found.
            earendel.module_api.errors.InvalidRuleException if the actions are invalid.
        """
    ...
    async def lookup_room_alias(self, room_alias: str) -> tuple[str, list[str]]:
        ...
        Raises:
            EarendelError if room alias is invalid or could not be found.
        """
    ...
    async def create_room(
        ...
        Raises:
            ...
            EarendelError if the user_id is invalid, room ID couldn't be stored, or
    ...
    async def delete_room(self, room_id: str) -> None:
        """
        Schedules the deletion of a room from Earendel's database.
    ...
    async def set_displayname(
        ...
        Added in Earendel v1.76.0.

        (Earendel Developer note: All future arguments should be kwargs-only
        due to https://github.com/sudhantechie/earendel/issues/19546)
        ...
        Args:
            ...
            deactivation:
                **deprecated since v1.150.0**
                ...
                Tracked by https://github.com/sudhantechie/earendel/issues/19546
