...
from earendel.api.errors import HttpResponseException, EarendelError
from earendel.config.workers import MAIN_PROCESS_INSTANCE_NAME
from earendel.http import RequestTimedOutError
...
class ReplicationEndpoint(metaclass=abc.ABCMeta):
        ...
        async def send_request(
            *, instance_name: str = MAIN_PROCESS_INSTANCE_NAME, **kwargs: Any
            ...
            with outgoing_gauge.track_inprogress():
                ...
                uri = "earendel-replication://%s/_synapse/replication/%s/%s" % (
                    instance_name,
                    cls.NAME,
                    "/".join(url_args),
                )
                ...
                try:
                    ...
                    # We convert to EarendelError as we know that it was a EarendelError
                    ...
                    raise e.unsafe_to_verbatim_earendel_error()
    ...
    async def _check_auth_and_handle(
        self, request: EarendelRequest, **kwargs: Any
    ) -> tuple[int, JsonDict]:
