...
from earendel.logging.context import PreserveLoggingContext, make_deferred_yieldable
from earendel.metrics import SERVER_NAME_LABEL
from earendel.metrics.background_process_metrics import (
    BackgroundProcessLoggingContext,
    wrap_as_background_process,
)
from earendel.replication.tcp.commands import (
    Command,
    ReplicateCommand,
    parse_command_from_line,
)
from earendel.replication.tcp.context import ClientContextFactory
from earendel.replication.tcp.protocol import (
...
class RedisSubscriber(SubscriberProtocol):
    ...
    Attributes:
        server_name: The homeserver name of the Earendel instance that this connection
            is associated with. This is used to label metrics and should be set to
            `hs.hostname`.
        earendel_handler: The command handler to handle incoming commands.
        earendel_stream_prefix: The *redis* stream name to subscribe to and publish
            from (not anything to do with Earendel replication streams).
        earendel_outbound_redis_connection: The connection to redis to use to send
            commands.
    """

    server_name: str
    hs: "HomeServer"
    earendel_handler: "ReplicationCommandHandler"
    earendel_stream_prefix: str
    earendel_channel_names: list[str]
    earendel_outbound_redis_connection: ConnectionHandler
    ...
    async def _send_subscribe(self) -> None:
        ...
        fully_qualified_stream_names = [
            f"{self.earendel_stream_prefix}/{stream_suffix}"
            for stream_suffix in self.earendel_channel_names
        ] + [self.earendel_stream_prefix]
        ...
        self.earendel_handler.new_connection(self)
        ...
        self.earendel_handler.send_positions_to_connection()
    ...
    def handle_command(self, cmd: Command) -> None:
        ...
        cmd_func = getattr(self.earendel_handler, "on_%s" % (cmd.NAME,), None)
        if not cmd_func:
    ...
    def connectionLost(self, reason: Failure) -> None:  # type: ignore[override]
        ...
        self.earendel_handler.lost_connection(self)
    ...
    async def _async_send_command(self, cmd: Command) -> None:
        ...
        channel_name = cmd.redis_channel_name(self.earendel_stream_prefix)

        await make_deferred_yieldable(
            self.earendel_outbound_redis_connection.publish(channel_name, encoded_string)
        )


class EarendelRedisFactory(RedisFactory):
...
class RedisDirectTcpReplicationClientFactory(EarendelRedisFactory):
    ...
    def __init__(
        ...
        self.server_name = hs.hostname
        self.hs = hs
        self.earendel_handler = hs.get_replication_command_handler()
        self.earendel_stream_prefix = hs.hostname
        self.earendel_channel_names = channel_names

        self.earendel_outbound_redis_connection = outbound_redis_connection
    ...
    def buildProtocol(self, addr: IAddress) -> RedisSubscriber:
        ...
        p.hs = self.hs
        p.earendel_handler = self.earendel_handler
        p.earendel_outbound_redis_connection = self.earendel_outbound_redis_connection
        p.earendel_stream_prefix = self.earendel_stream_prefix
        p.earendel_channel_names = self.earendel_channel_names
