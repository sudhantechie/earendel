...
from earendel.replication.tcp.protocol import IReplicationConnection
...
from earendel.replication.tcp.streams._base import (
    DeviceListsStream,
...
from earendel.util.background_queue import BackgroundQueue
...
inbound_rdata_count = Counter(
    "earendel_replication_tcp_protocol_inbound_rdata_count",
    "",
    labelnames=["stream_name", SERVER_NAME_LABEL],
)
user_sync_counter = Counter(
    "earendel_replication_tcp_resource_user_sync", "", labelnames=[SERVER_NAME_LABEL]
)
federation_ack_counter = Counter(
    "earendel_replication_tcp_resource_federation_ack",
    "",
    labelnames=[SERVER_NAME_LABEL],
)
# FIXME: Unused metric, remove if not needed.
remove_pusher_counter = Counter(
    "earendel_replication_tcp_resource_remove_pusher", "", labelnames=[SERVER_NAME_LABEL]
)

user_ip_cache_counter = Counter(
    "earendel_replication_tcp_resource_user_ip_cache", "", labelnames=[SERVER_NAME_LABEL]
)

tcp_resource_total_connections_gauge = LaterGauge(
    name="earendel_replication_tcp_resource_total_connections",
    desc="",
    labelnames=[SERVER_NAME_LABEL],
)

tcp_command_queue_gauge = LaterGauge(
    name="earendel_replication_tcp_command_queue",
...
class ReplicationCommandHandler:
    ...
    def start_replication(self, hs: "HomeServer") -> None:
        ...
        from earendel.replication.tcp.redis import RedisDirectTcpReplicationClientFactory
