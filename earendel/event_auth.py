...
from earendel.events import is_creator
from earendel.events.py_protocol import supports_msc4242_state_dag
from earendel.state import CREATE_KEY
...
def validate_event_for_room_version(event: "EventBase") -> None:
    ...
    Raises:
        EarendelError if there is a problem with the event
    """
...
def _check_size_limits(event: "EventBase") -> None:
    ...
    The codepoint size limits are only for Earendel compatibility.

    Raises:
        EventSizeError:
            when a size limit has been violated.

            unpersistable=True if Earendel never would have accepted the event and
                the PDU must NOT be persisted.

            unpersistable=False if a prior version of Earendel would have accepted the
    ...
    # Codepoint size check: Earendel always enforced these limits, so apply
    # them strictly.
...
def _check_power_levels(
    ...
    for k, v in user_list.items():
        try:
            ...
            raise EarendelError(400, "Not a valid user_id: %s" % (k,))

        try:
            int(v)
        except Exception:
            raise EarendelError(400, "Not a valid power level: %s" % (v,))
        ...
        for k, v in event.content.items():
            if k in {
                ...
                if type(v) is not int:  # noqa: E721
                    raise EarendelError(400, f"{v!r} must be an integer.")
