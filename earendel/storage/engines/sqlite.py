...
from earendel.storage.engines._base import AUTO_INCREMENT_PRIMARY_KEYPLACEHOLDER
from earendel.storage.types import Cursor

if TYPE_CHECKING:
    from earendel.storage.database import LoggingDatabaseConnection
...
class Sqlite3Engine(BaseDatabaseEngine[sqlite3.Connection, sqlite3.Cursor]):
    ...
    def check_database(
        ...
        if not allow_outdated_version:
            # Earendel is untested against older SQLite versions, and we don't want
            # to let users upgrade to a version of Earendel with broken support for their
            # sqlite version, because it risks leaving them with a half-upgraded db.
            if sqlite3.sqlite_version_info < (3, 37, 2):
                raise RuntimeError("Earendel requires sqlite 3.37.2 or above.")
