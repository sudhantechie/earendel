...
from earendel.api.room_versions import KNOWN_ROOM_VERSIONS
from earendel.crypto.event_signing import (
...
from earendel.storage.databases.main.cache import CacheInvalidationWorkerStore
from earendel.storage.databases.main.events import (
...
from earendel.storage.databases.main.events_worker import (
    DatabaseCorruptionError,
    EventRedactBehaviour,
    InvalidEventError,
)
from earendel.storage.databases.main.state_deltas import StateDeltasStore
from earendel.storage.databases.main.stream import StreamWorkerStore
from earendel.storage.engines import PostgresEngine
from earendel.storage.types import Cursor
from earendel.earendel_rust.events import redact_event
from earendel.types import JsonDict, RoomStreamToken, StateMap, StrCollection
from earendel.types.handlers import SLIDING_SYNC_DEFAULT_BUMP_EVENT_TYPES
...
from earendel.util.json import json_encoder
...
class EventsBackgroundUpdatesStore(
    ...
    def __init__(
        ...
        # https://github.com/sudhantechie/earendel/issues/17623)
    ...
    async def _cleanup_extremities_bg_update(
        ...
        Mainly used to deal with the aftermath of https://github.com/sudhantechie/earendel/issues/5269.
        """
        ...
        def _cleanup_extremities_bg_update_txn(txn: LoggingTransaction) -> int:
            ...
            logger.info(
                "Deleted %d forward extremities of %d checked, to clean up matrix-org/earendel#5269",
                deleted,
                len(original_set),
            )
    ...
    async def _sliding_sync_membership_snapshots_bg_update(
        ...
        def _find_memberships_to_update_txn(
            ...
            if initial_phase:
                ...
                # https://github.com/sudhantechie/earendel/issues/6983) where we don't have
        ...
        for (
            ...
            # There are some old out-of-band memberships (before
            # https://github.com/sudhantechie/earendel/issues/6983) where we don't have the
...
def _resolve_stale_data_in_sliding_sync_tables(
    ...
    This accounts for when someone downgrades their Earendel version and then upgrades it
...
def _resolve_stale_data_in_sliding_sync_joined_rooms_table(
    ...
    kicks-off the background update to catch-up with what we missed while Earendel was
    downgraded.
    ...
    if row is not None:
        ...
        # the expected normal scenario for people who have not downgraded their Earendel
        # version.
        if not room_rows:
            return
    ...
    # Now kick-off the background update to catch-up with what we missed while Earendel
    # was downgraded.
    #
    ...
    # on their server (so the normal background update completes), downgrade Earendel
...
def _resolve_stale_data_in_sliding_sync_membership_snapshots_table(
    ...
    and kicks-off the background update to catch-up with what we missed while Earendel
    was downgraded.
    ...
    if row is not None:
        ...
        # set out-of-band, there is no way to tell whether it was set while Earendel was
    ...
    # Earendel versions, join and create some new rooms, and upgrade again.
    #
