...
from earendel.api.room_versions import RoomVersion, RoomVersions
from earendel.config.homeserver import HomeServerConfig
from earendel.events import EventBase
from earendel.replication.tcp.streams._base import QuarantinedMediaStream
from earendel.replication.tcp.streams.partial_state import UnPartialStatedRoomStream
from earendel.storage._base import (
    db_to_json,
...
from earendel.util.stringutils import MXC_REGEX
...
class RatelimitOverride:
    # n.b. elsewhere in Earendel messages_per_second is represented as a float, but it is
...
class RoomWorkerStore(CacheInvalidationWorkerStore):
    ...
    async def _flag_existing_quarantined_media(
        ...
        # non-issues for us.
        #
        # Another similar issue is if Earendel is downgraded partway through the background
        # update then has more media quarantined. If Earendel is later upgraded again, the
        ...
        # See https://github.com/sudhantechie/earendel/issues/19672 for more details.
    ...
    async def get_quarantined_media_changes(
        ...
        Raises:
            EarendelError: If waiting for `to_id` took too long.
        """
        if to_id < from_id:
