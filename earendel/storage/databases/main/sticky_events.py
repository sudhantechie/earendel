...
from earendel.api.errors import AuthError
from earendel.events import EventBase
from earendel.events.snapshot import EventPersistencePair
from earendel.replication.tcp.streams._base import StickyEventsStream
...
from earendel.types import StateKey
...
class StickyEventsWorkerStore(StateGroupWorkerStore, CacheInvalidationWorkerStore):
    def __init__(
        ...
        if hs.config.experimental.msc4354_enabled and isinstance(
            ...
            if sqlite3.sqlite_version_info < (3, 40, 0):
                raise RuntimeError(
                    f"Experimental MSC4354 Sticky Events enabled but SQLite3 version is too old: {sqlite3.sqlite_version_info}, must be at least 3.40. Disable MSC4354 Sticky Events, switch to Postgres, or upgrade SQLite. See https://github.com/sudhantechie/earendel/issues/19428"
                )
    ...
    async def get_updated_sticky_events(
        ...
        if not self.hs.config.experimental.msc4354_enabled:
            ...
            # See: https://github.com/sudhantechie/earendel/issues/19428
            return []
    ...
    def insert_sticky_events_txn(
        ...
        Skips inserting events:
            ...
            - if they are considered spammy by a Earendel spam checker module;
