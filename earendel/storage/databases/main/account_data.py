...
from earendel.api.constants import AccountDataTypes
from earendel.api.errors import Codes, EarendelError
from earendel.replication.tcp.streams import AccountDataStream
from earendel.storage._base import db_to_json
from earendel.storage.admin_client_config import AdminClientConfig
...
from earendel.storage.invite_rule import (
...
from earendel.types import JsonDict, JsonMapping
...
class AccountDataWorkerStore(PushRulesWorkerStore, CacheInvalidationWorkerStore):
    ...
    async def get_admin_client_config_for_user(self, user_id: str) -> AdminClientConfig:
        ...
        The admin client config contains Earendel-specific settings that clients running
        ...
        """
        data = await self.get_global_account_data_by_type_for_user(
            user_id, AccountDataTypes.EAREENDEL_ADMIN_CLIENT_CONFIG
        )
    ...
    def _add_account_data_for_user(
        ...
        if user_id in currently_ignored_users:
            raise EarendelError(400, "You cannot ignore yourself", Codes.INVALID_PARAM)
