...
from earendel.api.constants import EventContentFields
...
from earendel.replication.tcp.streams import ToDeviceStream
from earendel.storage._base import SQLBaseStore, db_to_json
...
from earendel.storage.util.id_generators import MultiWriterIdGenerator
        ...
        def get_device_messages_txn(
            ...
            for row in txn:
                ...
                with start_active_span("get_to_device_message"):
                    set_tag(EarendelTags.TO_DEVICE_TYPE, message_dict["type"])
                    set_tag(EarendelTags.TO_DEVICE_SENDER, message_dict["sender"])
                    set_tag(EarendelTags.TO_DEVICE_RECIPIENT, user_id)
                    set_tag(EarendelTags.TO_DEVICE_RECIPIENT_DEVICE, device_id)
                    set_tag(
                        EarendelTags.TO_DEVICE_MSGID,
        ...
        def get_device_messages_txn(
            ...
            for row in txn:
                ...
                with start_active_span("get_to_device_message"):
                    set_tag(EarendelTags.TO_DEVICE_TYPE, message_dict["type"])
                    set_tag(EarendelTags.TO_DEVICE_SENDER, message_dict["sender"])
                    set_tag(EarendelTags.TO_DEVICE_RECIPIENT, recipient_user_id)
                    set_tag(EarendelTags.TO_DEVICE_RECIPIENT_DEVICE, recipient_device_id)
                    set_tag(
                        EarendelTags.TO_DEVICE_MSGID,
                        message_dict["content"].get(EventContentFields.TO_DEVICE_MSGID),
                    )
                ...
                for user_id, messages_by_device in edu["messages"].items():
                    for device_id, msg in messages_by_device.items():
                        with start_active_span("store_outgoing_to_device_message"):
                            set_tag(EarendelTags.TO_DEVICE_EDU_ID, edu["sender"])
                            set_tag(EarendelTags.TO_DEVICE_EDU_ID, edu["message_id"])
                            set_tag(EarendelTags.TO_DEVICE_TYPE, edu["type"])
                            set_tag(EarendelTags.TO_DEVICE_RECIPIENT, user_id)
                            set_tag(EarendelTags.TO_DEVICE_RECIPIENT_DEVICE, device_id)
                            set_tag(
                                EarendelTags.TO_DEVICE_MSGID,
...
def _serialize_to_device_message(
    *, user_id: str, device_id: str, msg: JsonDict
    ...
    with start_active_span("serialise_to_device_message"):
        ...
        set_tag(EarendelTags.TO_DEVICE_TYPE, msg["type"])
        set_tag(EarendelTags.TO_DEVICE_SENDER, msg["sender"])
        set_tag(EarendelTags.TO_DEVICE_RECIPIENT, user_id)
        set_tag(EarendelTags.TO_DEVICE_RECIPIENT_DEVICE, device_id)
        set_tag(EarendelTags.TO_DEVICE_MSGID, msgid)
