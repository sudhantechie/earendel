...
from earendel.api.constants import UserTypes
...
from earendel.storage.util.id_generators import IdGenerator
from earendel.storage.util.sequence import build_sequence_generator
from earendel.types import JsonDict, StrCollection, UserID, UserInfo
...
class RegistrationWorkerStore(StatsStore, CacheInvalidationWorkerStore):
    ...
    def _record_user_external_id_txn(
        ...
        against configured IdPs as Earendel does not know its relationship to
    ...
    async def generate_registration_token(self, length: int, chars: str) -> str | None:
        ...
        Raises:
            EarendelError if a unique registration token could still not be
