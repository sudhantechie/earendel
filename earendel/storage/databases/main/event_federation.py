...
from earendel.api.constants import MAX_DEPTH
from earendel.api.errors import StoreError
from earendel.api.room_versions import EventFormatVersions, RoomVersion
...
from earendel.storage.background_updates import ForeignKeyConstraint
...
from earendel.storage.engines import PostgresEngine, Sqlite3Engine
from earendel.types import JsonDict, StrCollection
from earendel.util.caches.descriptors import cached
...
oldest_pdu_in_federation_staging = Gauge(
    "earendel_federation_server_oldest_inbound_pdu_in_staging",
...
number_pdus_in_federation_queue = Gauge(
    "earendel_federation_server_number_inbound_pdu_in_staging",
    "The total number of events in the inbound federation staging",
    labelnames=[SERVER_NAME_LABEL],
)

pdus_pruned_from_federation_queue = Counter(
    "earendel_federation_server_number_inbound_pdu_pruned",
...
# importance other than knowing earendel can support it (given canonical json
# requires it).
...
class EventFederationWorkerStore(
    ...
    def _fixup_auth_chain_difference_sets(
        ...
        # `earendel.state.v2_get_auth_chain_difference`. We uses the observation
