...
from earendel.storage.engines import (
    BaseDatabaseEngine,
...
https://github.com/sudhantechie/earendel.

A temporary workaround to fix this error is to shut down Earendel (including
...
class SequenceGenerator(metaclass=abc.ABCMeta):
    ...
    def check_consistency(
        ...
        sync with the table, e.g. if Earendel gets rolled back to a previous
...
class LocalSequenceGenerator(SequenceGenerator):
    ...
    def get_next_id_txn(self, txn: Cursor) -> int:
        ...
        # we are a single process earendel.
        with self._lock:
