...
if TYPE_CHECKING:
    from earendel.notifier import ReplicationNotifier
...
class MultiWriterIdGenerator(AbstractStreamIdGenerator):
    ...
    Earendel only supports monolith mode when SQLite is the database driver.
    ...
    Warning: Streams using this generator start at ID 2, because ID 1 is always assumed
        ...
        DO NOT USE `START WITH 2` FOR THIS PURPOSE:
            see https://github.com/sudhantechie/earendel/issues/18712
    ...
    def _load_current_ids(
        ...
        # like if we rolled back Earendel, the stream_positions table might not be up to
    ...
    def get_current_token_for_writer(self, instance_name: str) -> int:
        ...
        # persisted up to position. This stops Earendel from doing a full table
    ...
    def _add_persisted_position(self, new_id: int) -> None:
        ...
        # Hacky debug logging to attempt to trace https://github.com/sudhantechie/earendel/issues/19795.
