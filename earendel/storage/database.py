...
from earendel.metrics import SERVER_NAME_LABEL, register_threadpool
from earendel.storage.background_updates import BackgroundUpdater
from earendel.storage.engines import BaseDatabaseEngine, PostgresEngine, Sqlite3Engine
from earendel.storage.types import Connection, Cursor, SQLQueryParameters
from earendel.types import StrCollection
from earendel.util.async_helpers import delay_cancellation
...
sql_logger = logging.getLogger("earendel.storage.SQL")
transaction_logger = logging.getLogger("earendel.storage.txn")
perf_logger = logging.getLogger("earendel.storage.TIME")
...
sql_scheduling_timer = Histogram(
    "earendel_storage_schedule_time", "sec", labelnames=[SERVER_NAME_LABEL]
)

sql_query_timer = Histogram(
    "earendel_storage_query_time", "sec", labelnames=["verb", SERVER_NAME_LABEL]
)
sql_txn_count = Counter(
    "earendel_storage_transaction_time_count",
    "sec",
    labelnames=["desc", SERVER_NAME_LABEL],
)
sql_txn_duration = Counter(
    "earendel_storage_transaction_time_sum",
    "sec",
    labelnames=["desc", SERVER_NAME_LABEL],
)
            ...
            for attempt_number in range(1, MAX_NUMBER_OF_ATTEMPTS + 1):
                ...
                try:
                    with opentracing.start_active_span(
                        "db.txn",
                        tags={
                            opentracing.EarendelTags.DB_TXN_DESC: desc,
                            opentracing.EarendelTags.DB_TXN_ID: name,
                        },
                    ):
                ...
                raise AssertionError(
                    ...
                    "This is a logic error in Earendel itself."
                )
        except Exception as e:
