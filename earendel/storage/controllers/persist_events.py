...
from earendel.api.constants import EventTypes, Membership
...
from earendel.events.py_protocol import (
    MSC4242Event,
    all_supports_msc4242_state_dag,
)
from earendel.events.snapshot import EventContext, EventPersistencePair
...
from earendel.storage.controllers.state import StateStorageController
from earendel.storage.databases import Databases
from earendel.storage.databases.main.events import DeltaState
...
from earendel.util.async_helpers import ObservableDeferred, yieldable_gather_results
...
state_delta_counter = Counter(
    "earendel_storage_events_state_delta", "", labelnames=[SERVER_NAME_LABEL]
)

# The number of times we are recalculating state when there is only a
# single forward extremity
state_delta_single_event_counter = Counter(
    "earendel_storage_events_state_delta_single_event",
    "",
...
state_delta_reuse_delta_counter = Counter(
    "earendel_storage_events_state_delta_reuse_delta", "", labelnames=[SERVER_NAME_LABEL]
)

# The number of forward extremities for each new event.
forward_extremities_counter = Histogram(
    "earendel_storage_events_forward_extremities_persisted",
...
stale_forward_extremities_counter = Histogram(
    "earendel_storage_events_stale_forward_extremities_persisted",
...
msc4242_state_dag_forward_extremities_counter = Histogram(
    "earendel_storage_msc4242_state_dag_forward_extremities_persisted",
...
class EventsPersistenceStorageController:
    ...
    async def _calculate_new_state_dag_extremities(
        ...
        Raises:
            EarendelError: if the new events include unknown prev_state_events
