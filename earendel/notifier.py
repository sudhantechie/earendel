...
from earendel.api.constants import EduTypes, EventTypes, HistoryVisibility, Membership
...
from earendel.logging.opentracing import log_kv, start_active_span
...
from earendel.util.async_helpers import (
    timeout_deferred,
)
...
notified_events_counter = Counter(
    "earendel_notifier_notified_events", "", labelnames=[SERVER_NAME_LABEL]
)

users_woken_by_stream_counter = Counter(
    "earendel_notifier_users_woken_by_stream",
    "",
...
notifier_listeners_gauge = LaterGauge(
    name="earendel_notifier_listeners",
    desc="",
    labelnames=[SERVER_NAME_LABEL],
)

notifier_rooms_gauge = LaterGauge(
    name="earendel_notifier_rooms",
    desc="",
    labelnames=[SERVER_NAME_LABEL],
)
notifier_users_gauge = LaterGauge(
    name="earendel_notifier_users",
    desc="",
    labelnames=[SERVER_NAME_LABEL],
)
...
class Notifier:
    ...
    async def wait_for_stream_token(self, stream_token: StreamToken) -> bool:
        ...
        While it was possible for older Earendel versions to erroneously give out invalid
        future tokens, this is no longer the case and its considered a Earendel
