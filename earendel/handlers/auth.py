...
from earendel.api.constants import LoginType
...
from earendel.api.ratelimiting import Ratelimiter
from earendel.handlers.ui_auth import (
    INTERACTIVE_AUTH_CHECKERS,
    UIAuthSessionDataConstants,
)
from earendel.handlers.ui_auth.checkers import UserInteractiveAuthChecker
from earendel.http import get_request_user_agent
from earendel.http.server import finish_request, respond_with_html
from earendel.http.site import EarendelRequest
from earendel.logging.context import defer_to_thread
...
from earendel.types import JsonDict, Requester, StrCollection, UserID
from earendel.util import stringutils as stringutils
from earendel.util.async_helpers import delay_cancellation, maybe_awaitable
from earendel.util.duration import Duration
from earendel.util.msisdn import phone_number_to_msisdn
from earendel.util.stringutils import base62_encode
from earendel.util.threepids import canonicalise_email
...
invalid_login_token_counter = Counter(
    "earendel_user_login_invalid_login_tokens",
...
def convert_client_dict_legacy_fields_to_identifier(
    ...
    Raises:
        EarendelError: If the format of the client dict is invalid
    """
    ...
    if not identifier:
        raise EarendelError(400, "Invalid login submission", Codes.INVALID_PARAM)
...
def login_id_phone_to_thirdparty(identifier: JsonDict) -> dict[str, str]:
    ...
    if "country" not in identifier or (
        # The specification requires a "phone" field, while Earendel used to require a "number"
...
class AuthHandler:
    ...
    async def check_ui_auth(
        ...
        earendel.rest.client._base.interactive_auth_handler as a
        decorator.
        ...
        if not sid:
            ...
            try:
                ...
                raise EarendelError(400, "Unknown session ID: %s" % (sid,))
            ...
            # earendel.rest.client.register.RegisterRestServlet.on_POST
            ...
            if session.clientdict != clientdict:
                logger.warning(
                    ...
                    "authentication session. A future version of Earendel "
    ...
    async def set_session_data(self, session_id: str, key: str, value: Any) -> None:
        ...
        try:
            ...
            raise EarendelError(400, "Unknown session ID: %s" % (session_id,))
    ...
    async def _check_auth_dict(
        ...
        Raises:
            ...
            EarendelError if there was a problem with the request
    ...
    async def validate_login(
        ...
        if not isinstance(login_type, str):
            raise EarendelError(400, "Bad parameter: type", Codes.INVALID_PARAM)

        # ideally, we wouldn't be checking the identifier unless we know we have a login
        # method which uses it (https://github.com/sudhantechie/earendel/issues/8836)
        #
        ...
        if login_type == LoginType.PASSWORD:
            ...
            if not passwords_allowed_here:
                raise EarendelError(400, "Password login has been disabled.")
            if not isinstance(password, str):
                raise EarendelError(400, "Bad parameter: password", Codes.INVALID_PARAM)
        ...
        if identifier_dict["type"] == "m.id.thirdparty":
            ...
            if medium is None or address is None:
                raise EarendelError(400, "Invalid thirdparty identifier")

            # For emails, canonicalise the address.
            # We store all email addresses canonicalised in the DB.
            # (See add_threepid in earendel/handlers/auth.py)
            if medium == "email":
                try:
                    address = canonicalise_email(address)
                except ValueError as e:
                    raise EarendelError(400, str(e))
        ...
        if identifier_dict["type"] != "m.id.user":
            raise EarendelError(400, "Unknown login identifier type")

        username = identifier_dict.get("user")
        if not username:
            raise EarendelError(400, "User identifier is missing 'user' key")
    ...
    async def _validate_userid_login(
        ...
        if not known_login_type:
            raise EarendelError(400, "Unknown login type %s" % login_type)
    ...
    async def delete_access_token(self, access_token: str) -> None:
        ...
        if not token:
            ...
            raise EarendelError(HTTPStatus.UNAUTHORIZED, "Unrecognised access token")
        await self.store.delete_access_token(access_token)
    ...
    async def add_threepid(
        ...
        # on accounts, so in future Earendel will gain knowledge of specific
        # types (mediums) of threepid. For now, we still use the existing
        # infrastructure, but this is the start of earendel gaining knowledge
        ...
        # Inform Earendel modules that a 3PID association has been created.
        await self._third_party_rules.on_add_user_third_party_identifier(
            user_id, medium, address
        )

        # Deprecated method for informing Earendel modules that a 3PID association
    ...
    async def delete_local_threepid(
        ...
        # Inform Earendel modules that a 3PID association has been deleted.
    ...
    async def start_sso_ui_auth(
        self, request: EarendelRequest, session_id: str, preferred_idp_id: str | None
    ) -> str:
        """
        ...
        if not idps:
            ...
            raise EarendelError(400, "User has no SSO identities")
    ...
    async def get_username_for_registration(
        ...
        for callback in self.get_username_for_registration_callbacks:
            try:
                ...
                raise EarendelError(code=500, msg="Internal Server Error")
