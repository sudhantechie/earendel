...
from earendel.api.errors import SlidingSyncUnknownPosition
from earendel.api.room_versions import KNOWN_ROOM_VERSIONS
from earendel.events import StrippedStateEvent
from earendel.events.utils import parse_stripped_state_event
from earendel.logging.opentracing import start_active_span, trace
from earendel.storage.databases.main.sliding_sync import UPDATE_INTERVAL_LAST_USED_TS
from earendel.storage.databases.main.state import (
    ROOM_UNKNOWN_SENTINEL,
    Sentinel as StateSentinel,
)
from earendel.storage.databases.main.stream import CurrentStateDeltaMembership
...
from earendel.util import MutableOverlayMapping
...
class SlidingSyncRoomLists:
    ...
    async def get_room_membership_for_user_at_to_token(
        ...
        `room_memberships` table in Earendel. There isn't a way to tell when a room was
    ...
    async def filter_rooms_relevant_for_sync(
        ...
        - In order for bans/kicks to not show up in sync, you need to `/forget` those
          ...
          `forgotten` flag to the `room_memberships` table in Earendel. There isn't a way
        ...
        for invite_or_knock_event in invite_or_knock_events.values():
            ...
            if membership == Membership.INVITE:
                ...
                raise AssertionError(
                    f"Unexpected membership {membership} (this is a problem with Earendel itself)"
                )
    ...
    async def _bulk_get_partial_current_state_content_for_rooms(
        ...
        for room_id in room_ids_without_results:
            ...
            assert stripped_state_map is not Absent, (
                ...
                + "with that room_id. (this is a problem with Earendel itself)"
            )
