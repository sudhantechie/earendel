...
from earendel.api.constants import Direction, EventTypes, Membership
from earendel.events import EventBase
from earendel.events.utils import FilteredEvent, strip_event
from earendel.handlers.relations import BundledAggregations
from earendel.handlers.sliding_sync.extensions import SlidingSyncExtensionHandler
from earendel.handlers.sliding_sync.room_lists import (
    RoomsForUserType,
    SlidingSyncRoomLists,
)
from earendel.handlers.sliding_sync.store import SlidingSyncConnectionStore
...
from earendel.storage.roommember import (
    MemberSummary,
)
...
from earendel.types.handlers.sliding_sync import (
    HaveSentRoomFlag,
...
sync_processing_time = Histogram(
    "earendel_sliding_sync_processing_time",
...
class SlidingSyncHandler:
    ...
    async def current_sync_for_user(
        ...
        set_tag(EarendelTags.RESULT_PREFIX + "result", bool(sliding_sync_result))
        set_tag(EarendelTags.FUNC_ARG_PREFIX + "sync_config.user", user_id)
    ...
    async def get_room_sync_data(
        ...
        set_tag(
            EarendelTags.FUNC_ARG_PREFIX + "membership",
            room_membership_for_user_at_to_token.membership,
        )
        set_tag(
            EarendelTags.FUNC_ARG_PREFIX + "timeline_limit",
        ...
        if room_membership_for_user_at_to_token.membership not in (
            ...
            if StateValues.WILDCARD in room_sync_config.required_state_map.get(
                ...
                set_tag(
                    EarendelTags.FUNC_ARG_PREFIX + "required_state_wildcard",
                    True,
                )
            ...
            # client wanted.
                ...
                set_tag(
                    EarendelTags.FUNC_ARG_PREFIX + "required_state_wildcard_event_type",
                    True,
                )
                required_state_filter = StateFilter.all()
            else:
                ...
                set_tag(
                    EarendelTags.FUNC_ARG_PREFIX
                    + "required_state_wildcard_state_key_count",
                    num_wild_state_keys,
                )
                set_tag(
                    EarendelTags.FUNC_ARG_PREFIX + "required_state_lazy",
                    lazy_load_room_members,
                )
                set_tag(
                    EarendelTags.FUNC_ARG_PREFIX + "required_state_other_count",
                    num_others,
                )
        ...
        set_tag(EarendelTags.RESULT_PREFIX + "initial", initial)
    ...
    async def _get_bump_stamp(
        ...
        if (
            ...
            # (tracked by https://github.com/sudhantechie/earendel/issues/17623)
