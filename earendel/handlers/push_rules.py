...
from earendel.api.errors import EarendelError, UnrecognizedRequestError
from earendel.push.clientformat import format_push_rules_for_user
from earendel.storage.push_rule import RuleNotFoundException
from earendel.earendel_rust.push import get_base_rule_ids
from earendel.types import JsonDict, StreamKeyType, UserID
...
class PushRulesHandler:
    ...
    async def set_rule_attr(
        ...
        Raises:
            ...
            EarendelError(400) if the value is malformed.
        ...
        if spec.attr == "enabled":
            ...
            if not isinstance(val, bool):
                ...
                raise EarendelError(400, "Value for 'enabled' must be boolean")
            await self._main_store.set_push_rule_enabled(
                user_id, namespaced_rule_id, val, is_default_rule
            )
        elif spec.attr == "actions":
            if not isinstance(val, dict):
                raise EarendelError(400, "Value must be a dict")
            actions = val.get("actions")
            if not isinstance(actions, list):
                raise EarendelError(400, "Value for 'actions' must be dict")
