...
from earendel.api.constants import LoginType, ProfileFields
from earendel.api.errors import Codes, NotFoundError, RedirectException, EarendelError
from earendel.config.sso import SsoAttributeRequirement
from earendel.handlers.register import init_counters_for_auth_provider
from earendel.handlers.ui_auth import UIAuthSessionDataConstants
from earendel.http import get_request_user_agent
from earendel.http.server import respond_with_html, respond_with_redirect
...
class SsoIdentityProvider(Protocol):
    ...
    Earendel supports various implementations of IdPs, including OpenID Connect, SAML,
    and CAS.
...
class SsoHandler:
    ...
    def render_error(
        ...
        be found under `earendel/res/templates/sso_error.html`.
    ...
    async def _register_mapped_user(
        ...
        Raises:
            ...
            a EarendelError with code 400 and errcode Codes.USER_IN_USE if the localpart
            is already taken.
        """
    ...
    async def complete_sso_ui_auth_request(
        ...
        recentish earendel versions, but may not be for older users.
    ...
    def get_mapping_session(self, session_id: str) -> UsernameMappingSession:
        ...
        If it is not found, raises a EarendelError with an http code of 400
        ...
        Raises:
            EarendelError if the session is not found/has expired
        """
        ...
        raise EarendelError(400, "unknown session")
    ...
    async def check_username_availability(
        ...
        Raises:
            EarendelError if the localpart is invalid or the session is unknown
        """
        ...
        if contains_invalid_mxid_characters(localpart):
            raise EarendelError(400, "localpart is invalid: %s" % (localpart,))
    ...
    async def handle_terms_accepted(
        self, request: EarendelRequest, session_id: str, terms_version: str
    ) -> None:
...
def get_username_mapping_session_cookie_from_request(request: IRequest) -> str:
    ...
    Raises a EarendelError if the cookie isn't found
    """
    session_id = request.getCookie(USERNAME_MAPPING_SESSION_COOKIE_NAME)
    if not session_id:
        raise EarendelError(code=400, msg="missing session_id")
