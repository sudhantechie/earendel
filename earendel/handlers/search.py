...
from earendel.types import JsonDict, Requester, StrCollection, StreamKeyType, UserID
...
class SearchHandler:
    ...
    async def search(
        ...
        if not self.hs.config.server.enable_search:
            raise EarendelError(400, "Search is disabled on this homeserver")
        ...
        if batch:
            try:
                ...
                raise EarendelError(400, "Invalid batch")
        ...
        try:
            ...
            raise EarendelError(400, "Invalid search query")

        if order_by not in ("rank", "recent"):
            raise EarendelError(400, "Invalid order by: %r" % (order_by,))
    ...
    async def _search_by_recent(
        ...
        # But only go around 5 times since otherwise earendel will be sad.
