...
from earendel.config.oidc import OidcProviderClientSecretJwtKey, OidcProviderConfig
from earendel.handlers.sso import MappingException, SsoSetupError, UserAttributes
from earendel.http.server import finish_request
from earendel.http.servlet import parse_string
from earendel.http.site import EarendelRequest
from earendel.logging.context import make_deferred_yieldable
from earendel.metrics.background_process_metrics import wrap_as_background_process
from earendel.types import JsonDict, UserID, map_username_to_mxid_localpart
from earendel.util.caches.cached_call import RetryOnExceptionCachedCall
from earendel.util.clock import Clock
from earendel.util.json import json_decoder
from earendel.util.macaroons import MacaroonGenerator, OidcSessionData
from earendel.util.templates import _localpart_from_email_filter
...
class OidcHandler:
    ...
    async def handle_oidc_callback(self, request: EarendelRequest) -> None:
        """Handle an incoming request to /_synapse/client/oidc/callback

        Since we might want to display OIDC-related errors in a user-friendly
        way, we don't raise EarendelError from here. Instead, we call
        ...
        if b"error" in request.args:
            ...
            # either the provider misbehaving or Earendel being misconfigured.
    ...
    async def handle_backchannel_logout(self, request: EarendelRequest) -> None:
        ...
        if logout_token is None:
            raise EarendelError(400, "Missing logout_token in request")
        ...
        try:
            ...
            raise EarendelError(400, "Invalid logout_token in request")
        ...
        try:
            ...
            raise EarendelError(400, "Invalid logout_token payload in request")
        ...
        try:
            ...
            raise EarendelError(400, "Invalid issuer/audience in logout_token")
        ...
        if oidc_provider is None:
            raise EarendelError(400, "Could not find the OP that issued this event")
...
class OidcProvider:
            ...
            if not self._config.backchannel_logout_ignore_sub:
                ...
                try:
                    ...
                    logger.warning(
                        ...
                        "and you want Earendel to ignore the `sub` claim in OIDC "
    ...
    def _uses_access_token(self) -> bool:
        ...
        """
        # Currently, Earendel only uses the access_token to fetch user metadata
    ...
    async def handle_redirect_request(
        ...
        # to allow TI-Messenger deployments using Earendel to satisfy this
        # external spec.
        #
        # See https://github.com/sudhantechie/earendel/pull/18109 for more context.
    ...
    async def handle_oidc_callback(
        self, request: EarendelRequest, session_data: "OidcSessionData", code: str
    ) -> None:
        """Handle an incoming request to /_synapse/client/oidc/callback

        By this time we have already validated the session on the earendel side, and
    ...
    def _remote_id_from_userinfo(self, userinfo: UserInfo) -> str:
        ...
        # Some OIDC providers use integer IDs, but Earendel expects external IDs
        # to be strings.
        return str(remote_user_id)

    async def handle_backchannel_logout(
        self, request: EarendelRequest, logout_token: str
    ) -> None:
        """Handle an incoming request to /_synapse/client/oidc/backchannel_logout
        ...
        # This is not that important for now since Earendel is registered
        ...
        # support dynamic registration in Earendel at some point.
        ...
        # Which means the audience should contain Earendel's client_id and the
        ...
        try:
            ...
            raise EarendelError(400, "Invalid logout_token")
