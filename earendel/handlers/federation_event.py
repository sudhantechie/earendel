...
from earendel import event_auth
from earendel.api.constants import (
    EventContentFields,
    EventTypes,
    GuestAccess,
    Membership,
...
from earendel.api.errors import (
    AuthError,
    Codes,
    EventSizeError,
    FederationError,
...
from earendel.api.room_versions import KNOWN_ROOM_VERSIONS, RoomVersion, RoomVersions
from earendel.event_auth import (
...
from earendel.events.snapshot import (
    EventContext,
    EventPersistencePair,
    UnpersistedEventContextBase,
)
from earendel.federation.federation_client import InvalidResponseError, PulledPduInfo
from earendel.logging.context import nested_logging_context
from earendel.logging.opentracing import (
    EarendelTags,
    set_tag,
    start_active_span,
    tag_args,
...
from earendel.metrics import SERVER_NAME_LABEL
from earendel.replication.http.federation import (
    ReplicationFederationSendEventsRestServlet,
)
from earendel.state import StateResolutionStore
from earendel.storage.databases.main.events_worker import EventRedactBehaviour
...
from earendel.util.async_helpers import Linearizer, concurrently_execute
from earendel.util.duration import Duration
from earendel.util.iterutils import batch_iter, partition, sorted_topologically
from earendel.util.retryutils import NotRetryingDestination
from earendel.util.stringutils import shortstr
...
soft_failed_event_counter = Counter(
    "earendel_federation_soft_failed_events_total",
...
backfill_processing_after_timer = Histogram(
    "earendel_federation_backfill_processing_after_time_seconds",
    "sec",
    labelnames=[SERVER_NAME_LABEL],
    buckets=(
        0.1,
...
class FederationEventHandler:
    ...
    async def on_send_membership_event(
        ...
        Raises:
            ...
            EarendelError if the event is not accepted into the room
        ...
        if get_domain_from_id(event.sender) != origin:
            ...
            raise EarendelError(403, "User not from origin", Codes.FORBIDDEN)

        if event.sender != event.state_key:
            raise EarendelError(400, "state_key and sender must match", Codes.BAD_JSON)
    ...
    async def process_remote_join(
        ...
        Raises:
            EarendelError if the response is in some way invalid.
        ...
        if create_event is None:
            ...
            raise EarendelError(400, "No create event in state")
        ...
        if room_version.identifier != room_version_id:
            raise EarendelError(400, "Room version mismatch")
        ...
        with nested_logging_context(suffix=event.event_id):
            ...
            await self._check_event_auth(origin, event, context)
            if context.rejected:
                raise EarendelError(403, "Join event was rejected")
    ...
    async def backfill(
        ...
        this method throws a EarendelError.
        ...
        if self._is_mine_server_name(dest):
            raise EarendelError(400, "Can't backfill from self.")
    ...
    async def _get_missing_events_for_pdu(
        ...
        # https://github.com/sudhantechie/earendel/issues/1733.
        ...
        # causing additional state resolution?  See https://github.com/sudhantechie/earendel/issues/1760.
        # However, fetching state doesn't hold the linearizer lock
        # apparently.
        #
        # see https://github.com/sudhantechie/earendel/pull/1744
        #
        # ----
        #
        ...
        #   https://github.com/sudhantechie/earendel/pull/3456) reject any PDUs where the
    ...
    async def _process_pulled_events(
        ...
        """
        set_tag(
            EarendelTags.FUNC_ARG_PREFIX + "event_ids",
            str([event.event_id for event in events]),
        )
        set_tag(
            EarendelTags.FUNC_ARG_PREFIX + "event_ids.length",
            str(len(events)),
        )
        set_tag(EarendelTags.FUNC_ARG_PREFIX + "backfilled", str(backfilled))
        logger.debug(
        ...
        # https://github.com/sudhantechie/earendel/issues/13476
        ...
        set_tag(
            EarendelTags.RESULT_PREFIX + "new_events.length",
            str(len(new_events)),
        )
        ...
        set_tag(
            EarendelTags.FUNC_ARG_PREFIX + "events_with_failed_pull_attempts",
            str(event_ids_with_failed_pull_attempts),
        )
        set_tag(
            EarendelTags.RESULT_PREFIX + "events_with_failed_pull_attempts.length",
            str(len(events_with_failed_pull_attempts)),
        )
        set_tag(
            EarendelTags.FUNC_ARG_PREFIX + "fresh_events",
            str([event.event_id for event in fresh_events]),
        )
        set_tag(
            EarendelTags.RESULT_PREFIX + "fresh_events.length",
    ...
    async def _get_state_ids_after_missing_prev_event(
        ...
        #
        # Tracked by https://github.com/sudhantechie/earendel/issues/13618
        (
            state_event_ids,
            auth_event_ids,
        ...
        set_tag(
            EarendelTags.RESULT_PREFIX + "missing_auth_event_ids",
            str(missing_auth_event_ids),
        )
        set_tag(
            EarendelTags.RESULT_PREFIX + "missing_auth_event_ids.length",
            str(len(missing_auth_event_ids)),
        )
        set_tag(
            EarendelTags.RESULT_PREFIX + "missing_desired_event_ids",
            str(missing_desired_event_ids),
        )
        set_tag(
            EarendelTags.RESULT_PREFIX + "missing_desired_event_ids.length",
        ...
        if failed_to_fetch:
            ...
            set_tag(
                EarendelTags.RESULT_PREFIX + "failed_to_fetch",
                str(failed_to_fetch),
            )
            set_tag(
                EarendelTags.RESULT_PREFIX + "failed_to_fetch.length",
        ...
        async def prep(event: EventBase) -> None:
            with nested_logging_context(suffix=event.event_id):
                ...
                try:
                    ...
                    # as rejected, for moderate compatibility with older Earendel
                    # versions.
    ...
    async def _check_event_auth(
        ...
        set_tag(
            EarendelTags.RESULT_PREFIX + "claimed_auth_events",
            str([ev.event_id for ev in claimed_auth_events]),
        )
        set_tag(
            EarendelTags.RESULT_PREFIX + "claimed_auth_events.length",
        ...
        if instance != self._instance_name:
            ...
            if not backfilled:  # Never notify for backfilled events
                with start_active_span("notify_persisted_events"):
                    set_tag(
                        EarendelTags.RESULT_PREFIX + "event_ids",
                        str([ev.event_id for ev in events]),
                    )
                    set_tag(
                        EarendelTags.RESULT_PREFIX + "event_ids.length",
                        str(len(events)),
                    )
                    for event in events:
    ...
    def _sanity_check_event(self, ev: EventBase) -> None:
        ...
        Raises:
            EarendelError if the event does not pass muster
        """
        if len(ev.prev_event_ids()) > 20:
            ...
            raise EarendelError(HTTPStatus.BAD_REQUEST, "Too many prev_events")
        ...
        if len(ev.auth_event_ids()) > 10:
            ...
            raise EarendelError(HTTPStatus.BAD_REQUEST, "Too many auth_events")
