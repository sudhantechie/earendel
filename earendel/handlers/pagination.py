...
from earendel.util.async_helpers import ReadWriteLock
...
class PaginationHandler:
        ...
        async with self._worker_locks.acquire_multi_read_write_lock(
            ...
            if joined:
                if force:
                    ...
                    raise EarendelError(400, "Users are still joined to this room")
    ...
    async def start_shutdown_and_purge_room(
        ...
        """
        if len(await self.get_delete_tasks_by_room(room_id, only_active=True)) > 0:
            raise EarendelError(400, "Purge already in progress for %s" % (room_id,))
