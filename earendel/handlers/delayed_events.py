...
from earendel.api.constants import EventTypes, StickyEvent, StickyEventField
from earendel.api.errors import Codes, ShadowBanError, EarendelError
from earendel.api.ratelimiting import Ratelimiter
from earendel.config.workers import MAIN_PROCESS_INSTANCE_NAME
...
from earendel.replication.http.delayed_events import (
    ReplicationAddedDelayedEventRestServlet,
)
from earendel.storage.databases.main.delayed_events import (
    DelayedEventDetails,
...
class DelayedEventsHandler:
    ...
    async def _handle_state_deltas(self, deltas: list[StateDelta]) -> None:
        ...
        for delta in deltas:
            ...
            if sender_str is None:
                # An event exists, but the `sender` field was "null" and Earendel
    ...
    async def add(
        ...
        *,
        ...
        Raises:
            EarendelError: if the delayed event fails validation checks, or
        ...
        # See https://github.com/sudhantechie/earendel/issues/18021
    ...
    async def cancel(self, request: EarendelRequest, delay_id: str) -> None:
        """
    ...
    async def restart(self, request: EarendelRequest, delay_id: str) -> None:
        """
    ...
    async def send(self, request: EarendelRequest, delay_id: str) -> None:
        """
    ...
    async def _mgmt_ratelimit(self, request: EarendelRequest) -> None:
        """
