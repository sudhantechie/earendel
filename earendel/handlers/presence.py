...
and persisted periodically. When Earendel starts each worker loads the current
...
import earendel.metrics
from earendel.api.constants import EduTypes, EventTypes, Membership, PresenceState
from earendel.api.errors import EarendelError
from earendel.api.presence import UserDevicePresenceState, UserPresenceState
from earendel.appservice import ApplicationService
from earendel.events.presence_router import PresenceRouter
from earendel.logging.context import run_in_background
from earendel.metrics import SERVER_NAME_LABEL, LaterGauge
from earendel.metrics.background_process_metrics import (
    wrap_as_background_process,
)
from earendel.replication.http.presence import (
    ReplicationBumpPresenceActiveTime,
    ReplicationPresenceSetState,
)
from earendel.replication.http.streams import ReplicationGetStreamUpdates
from earendel.replication.tcp.commands import ClearUserSyncsCommand
from earendel.replication.tcp.streams import PresenceFederationStream, PresenceStream
from earendel.storage.databases.main import DataStore
from earendel.storage.databases.main.state_deltas import StateDelta
from earendel.streams import EventSource
...
from earendel.util.wheel_timer import WheelTimer
...
notified_presence_counter = Counter(
    "earendel_handler_presence_notified_presence", "", labelnames=[SERVER_NAME_LABEL]
)
federation_presence_out_counter = Counter(
    "earendel_handler_presence_federation_presence_out",
    "",
    labelnames=[SERVER_NAME_LABEL],
)
presence_updates_counter = Counter(
    "earendel_handler_presence_presence_updates", "", labelnames=[SERVER_NAME_LABEL]
)
timers_fired_counter = Counter(
    "earendel_handler_presence_timers_fired", "", labelnames=[SERVER_NAME_LABEL]
)
federation_presence_counter = Counter(
    "earendel_handler_presence_federation_presence", "", labelnames=[SERVER_NAME_LABEL]
)
bump_active_time_counter = Counter(
    "earendel_handler_presence_bump_active_time", "", labelnames=[SERVER_NAME_LABEL]
)

get_updates_counter = Counter(
    "earendel_handler_presence_get_updates", "", labelnames=["type", SERVER_NAME_LABEL]
)

notify_reason_counter = Counter(
    "earendel_handler_presence_notify_reason",
    "",
    labelnames=["locality", "reason", SERVER_NAME_LABEL],
)
state_transition_counter = Counter(
    "earendel_handler_presence_state_transition",
    "",
    labelnames=["locality", "from", "to", SERVER_NAME_LABEL],
)

presence_user_to_current_state_size_gauge = LaterGauge(
    name="earendel_handlers_presence_user_to_current_state_size",
    desc="",
    labelnames=[SERVER_NAME_LABEL],
)

presence_wheel_timer_size_gauge = LaterGauge(
    name="earendel_handlers_presence_wheel_timer_size",
    desc="",
    labelnames=[SERVER_NAME_LABEL],
)
...
# `earendel.config.server.DEFAULT_LAST_ACTIVE_GRANULARITY` and friends for the
# defaults).
    ...
    async def update_external_syncs_row(  # noqa: B027 (no-op by design)
        ...
        Args:
            process_id: An identifier for the process the users are
                syncing against. This allows earendel to process updates
...
class WorkerPresenceHandler(BasePresenceHandler):
    ...
    async def set_state(
        ...
        if presence not in self.VALID_PRESENCE:
            raise EarendelError(400, "Invalid presence state")
...
class PresenceHandler(BasePresenceHandler):
    ...
    async def _on_shutdown(self) -> None:
        ...
        earlier than they should when earendel is restarted. This affect of this
    ...
    async def _unsafe_process(self) -> None:
        ...
        while True:
            with Measure(
                ...
                earendel.metrics.event_processing_positions.labels(
    ...
    async def get_new_events(
        ...
        with Measure(
            ...
            if from_key == max_token:
                ...
                # doesn't return. C.f. https://github.com/sudhantechie/earendel/issues/5503.
                return [], max_token
