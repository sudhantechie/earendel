...
from earendel.api.urls import ConsentURIBuilder
from earendel.event_auth import validate_event_for_room_version
from earendel.events import EventBase, relation_from_event
from earendel.events.builder import EventBuilder
...
from earendel.replication.http.send_events import ReplicationSendEventsRestServlet
...
from earendel.util import log_failure, unwrapFirstError
from earendel.util.async_helpers import Linearizer, gather_results
from earendel.util.caches.expiringcache import ExpiringCache
from earendel.util.duration import Duration
from earendel.util.json import json_decoder, json_encoder
from earendel.util.metrics import measure_func
from earendel.visibility import get_effective_room_visibility_from_state
...
class MessageHandler:
    ...
    async def get_room_data(
        ...
        Raises:
            EarendelError or AuthError if the user is not in the room
        """
        (
            membership,
            membership_event_id,
    ...
    async def get_state_events(
        ...
        Args:
            ...
            at_token: the stream token of the at which we are requesting
                ...
                stream token, we raise a 403 EarendelError. If None, returns the current
...
class EventCreationHandler:
    ...
    async def create_event(
        ...
        # See `earendel.events.utils.EventClientSerializer.serialize_event`
        ...
        # earendel.handlers.directory pointless.
        ...
        for i in range(max_retries):
            try:
                ...
                if spam_check_result != self._spam_checker_module_callbacks.NOT_SPAM:
                    ...
                    # EarendelError (which is now deprecated).
                    raise EarendelError(
                        403,
                        spam_check_result,
                        Codes.FORBIDDEN,
    ...
    async def _validate_event_relation(self, event: EventBase) -> None:
        ...
        Raises:
            EarendelError if the event is invalid.
        """
        ...
        if parent_event:
            ...
            if parent_event.room_id != event.room_id:
                raise EarendelError(400, "Relations must be in the same room")
            ...
            if not await self.store.event_is_target_of_relation(relation.parent_id):
                ...
                raise EarendelError(400, "Can't send relation to unknown event")
        ...
        if relation.rel_type == RelationTypes.ANNOTATION:
            ...
            if aggregation_key is None:
                raise EarendelError(400, "Missing aggregation key")

            if len(aggregation_key) > 500:
                raise EarendelError(400, "Aggregation key is too long")
