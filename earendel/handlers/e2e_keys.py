...
from earendel.api.constants import EduTypes
from earendel.api.errors import CodeMessageException, Codes, NotFoundError, EarendelError
from earendel.handlers.device import DeviceWriterHandler
from earendel.logging.context import make_deferred_yieldable, run_in_background
from earendel.logging.opentracing import log_kv, set_tag, tag_args, trace
...
from earendel.util.retryutils import (
...
class E2eKeysHandler:
        ...
        async with self._query_devices_linearizer.queue((from_user_id, from_device_id)):
            ...
            if remote_queries:
                ...
                if invalid_cached_users:
                    ...
                    #       https://github.com/sudhantechie/earendel/issues/13651
    ...
    async def query_local_devices(
        ...
        for user_id, device_ids in query.items():
            ...
            if not self.is_mine(UserID.from_string(user_id)):
                ...
                set_tag("error", True)
                raise EarendelError(400, "Not a user here")
    ...
    async def upload_signing_keys_for_user(
        ...
        if not master_key:
            raise EarendelError(400, "No master key available", Codes.MISSING_PARAM)
        ...
        try:
            ...
            if "master_key" in keys:
                ...
                raise EarendelError(400, "Invalid master key", Codes.INVALID_PARAM)
            else:
                # the invalid key came from the database
                logger.error("Invalid master key found for user %s", user_id)
                raise EarendelError(500, "Invalid master key")
        ...
        if "self_signing_key" in keys:
            ...
            try:
                ...
                raise EarendelError(400, "Invalid self-signing key", Codes.INVALID_PARAM)
        if "user_signing_key" in keys:
            await self.store.set_e2e_cross_signing_key(
    ...
    async def upload_signatures_for_device_keys(
        ...
        Raises:
            EarendelError: if the signatures dict is not valid.
        """
        failures = {}
    ...
    async def _process_self_signatures(
        ...
        Raises:
            EarendelError: if the input is malformed
        """
        ...
        if not isinstance(signatures, dict):
            raise EarendelError(400, "Invalid parameter", Codes.INVALID_PARAM)
    ...
    def _check_master_key_signature(
        ...
        Raises:
            EarendelError: if a signature is invalid
        """
    ...
    async def _get_e2e_cross_signing_verify_key(
        ...
        Raises:
            ...
            EarendelError: if `user_id` is invalid
        """
    ...
    async def _delete_old_one_time_keys_task(
        ...
        Until Earendel 1.119, Earendel used to issue one-time-keys in a random order, leading to the possibility
...
def _check_cross_signing_key(
    ...
    if (
        ...
        raise EarendelError(400, ("Invalid %s key" % (key_type,)), Codes.INVALID_PARAM)
...
def _check_device_signature(
    ...
    Raises:
        EarendelError: if the signature was invalid or the sent device is not the
    ...
    if stripped_signed_device != stripped_stored_device:
        ...
        raise EarendelError(400, "Key does not match")

    try:
        verify_signed_json(signed_device, user_id, verify_key)
    except SignatureVerifyException:
        logger.debug("invalid signature on key")
        raise EarendelError(400, "Invalid signature", Codes.INVALID_SIGNATURE)
