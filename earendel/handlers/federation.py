...
from earendel.api.room_versions import KNOWN_ROOM_VERSIONS, RoomVersion
from earendel.crypto.event_signing import compute_event_signature
from earendel.event_auth import validate_event_for_room_version
from earendel.events import EventBase, event_exists_in_state_dag
from earendel.events.py_protocol import supports_msc4242_state_dag
from earendel.events.snapshot import EventContext, UnpersistedEventContextBase
from earendel.events.validator import EventValidator
from earendel.federation.federation_client import InvalidResponseError
from earendel.handlers.pagination import PURGE_PAGINATION_LOCK_NAME
from earendel.http.servlet import assert_params_in_dict
from earendel.logging.context import nested_logging_context
from earendel.logging.opentracing import EarendelTags, set_tag, tag_args, trace
from earendel.metrics import SERVER_NAME_LABEL
from earendel.module_api import NOT_SPAM
from earendel.storage.databases.main.events_worker import EventRedactBehaviour
from earendel.storage.invite_rule import InviteRule
from earendel.types import JsonDict, StrCollection, get_domain_from_id
from earendel.types.state import StateFilter
from earendel.util.async_helpers import Linearizer
from earendel.util.duration import Duration
from earendel.util.retryutils import NotRetryingDestination
from earendel.visibility import filter_events_for_server
...
backfill_processing_before_timer = Histogram(
    "earendel_federation_backfill_processing_before_time_seconds",
    "sec",
...
class FederationHandler:
    ...
    async def _maybe_backfill_inner(
        ...
        *,
        ...
        set_tag(
            EarendelTags.RESULT_PREFIX + "sorted_backfill_points",
            str(sorted_backfill_points),
        )
        set_tag(
            EarendelTags.RESULT_PREFIX + "sorted_backfill_points.length",
        ...
        set_tag(
            EarendelTags.RESULT_PREFIX + "extremities_to_request",
            str(extremities_to_request),
        )
        set_tag(
            EarendelTags.RESULT_PREFIX + "extremities_to_request.length",
        ...
        async def try_backfill(domains: StrCollection) -> bool:
            ...
            for dom in domains:
                ...
                try:
                    ...
                    continue
                except (EarendelError, InvalidResponseError) as e:
    ...
    async def send_invite(self, target_host: str, event: EventBase) -> EventBase:
        ...
        try:
            ...
            raise EarendelError(502, f"Can't connect to server {target_host}")
        ...
        try:
            ...
            async with self._is_partial_state_room_linearizer.queue(room_id):
                ...
                if ret.partial_state and not already_partial_state_room:
                    ...
                    #     https://github.com/sudhantechie/earendel/issues/13000
    ...
    async def do_knock(
        ...
        Raises:
            EarendelError: If the chosen remote server returns a 3xx/4xx code.
    ...
    async def on_invite_request(
        ...
        if event.state_key is None:
            raise EarendelError(400, "The invite event did not have a state key")

        is_blocked = await self.store.is_room_blocked(event.room_id)
        if is_blocked:
            raise EarendelError(403, "This room has been blocked on this server")

        if self.hs.config.server.block_non_admin_invites:
            raise EarendelError(403, "This server does not accept room invites")
        ...
        if event.type != EventTypes.Member or membership != Membership.INVITE:
            raise EarendelError(400, "The event was not an m.room.member invite event")
        ...
        if not self.is_mine_id(event.state_key):
            raise EarendelError(400, "The invite event must be for this server")

        # block any attempts to invite the server notices mxid
        if event.state_key == self._server_notices_mxid:
            raise EarendelError(HTTPStatus.FORBIDDEN, "Cannot invite this user")
    ...
    async def on_backfill_request(
        ...
        # Earendel asks for 100 events per backfill request. Do not allow more.
        limit = min(limit, 100)
    ...
    async def exchange_third_party_invite(
        ...
        if await self._event_auth_handler.is_host_in_room(room_id, self.hs.hostname):
            ...
            try:
                ...
                raise EarendelError(502, "Failed to forward third party invite")
    ...
    async def _check_signature(self, event: EventBase, context: EventContext) -> None:
        ...
        Raises:
            AuthError: if signature didn't match any keys, or key has been
                revoked,
            EarendelError: if a transient error meant a key couldn't be checked
                for revocation.
        """
    ...
    async def _check_key_revocation(self, public_key: str, url: str) -> None:
        ...
        try:
            ...
            raise EarendelError(502, "Third party certificate could not be checked")
    ...
    async def _sync_partial_state_room(
        ...
        #   https://github.com/sudhantechie/earendel/issues/12994
        #
        # TODO(faster_joins): what happens if we leave the room during a resync? if we
        #   really leave, that might mean we have difficulty getting the room state over
        #   federation.
        #   https://github.com/sudhantechie/earendel/issues/12802
                ...
                for attempt in itertools.count():
                    ...
                    try:
                        ...
                        if attempt == len(destinations) - 1:
                            ...
                            #   see https://github.com/sudhantechie/earendel/issues/13700
                            raise
