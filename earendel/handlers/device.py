...
from earendel.api.constants import EduTypes, EventTypes, Membership
...
from earendel.logging.opentracing import log_kv, set_tag, trace
from earendel.metrics import SERVER_NAME_LABEL
from earendel.metrics.background_process_metrics import (
    wrap_as_background_process,
)
from earendel.replication.http.devices import (
...
from earendel.storage.databases.main.client_ips import DeviceLastConnectionInfo
from earendel.storage.databases.main.roommember import EventIdMembership
...
device_list_conversion_lag_gauge = Gauge(
    "earendel_device_lists_changes_conversion_lag_seconds",
...
device_list_conversion_stream_lag_gauge = Gauge(
    "earendel_device_lists_changes_conversion_stream_lag",
...
def _check_device_name_length(name: str | None) -> None:
    ...
    Raises:
        EarendelError: if the device name is too long.
    """
...
class DeviceWriterHandler(DeviceHandler):
    ...
    def __init__(self, hs: "HomeServer"):
        ...
        # `earendel.app.generic_worker.FederationSenderHandler` when it sees it
            ...
            while True:
                ...
                for user_id, device_id, room_id, stream_id, opentracing_context in rows:
                    ...
                    if hosts and self.federation_sender:
                        ...
                        # memory usage.
                        # See https://github.com/sudhantechie/earendel/issues/12552.
                        # log_kv(
...
class DeviceListUpdater(DeviceListWorkerUpdater):
    ...
    async def _user_device_resync_returning_failed(
        ...
        try:
            ...
            # as otherwise earendel will 'forget' that its device list
        ...
        # inbound federation worker to the earendel master may time out
