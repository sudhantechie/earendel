...
import earendel.events.snapshot
...
from earendel.events.snapshot import UnpersistedEventContext
...
from earendel.rest.admin._base import assert_user_is_admin
...
from earendel.types.handlers import ShutdownRoomParams, ShutdownRoomResponse
from earendel.types.state import StateFilter
from earendel.util import stringutils
...
class RoomCreationHandler:
    ...
    async def _upgrade_room(
        ...
        tombstone_context: earendel.events.snapshot.EventContext,
        additional_creators: list[str] | None,
        creation_event_with_context: tuple[
            EventBase, earendel.events.snapshot.EventContext
        ]
        | None = None,
        auto_member: bool = False,
    ) -> str:
    ...
    async def create_room(
        ...
        if not isinstance(room_version_id, str):
            raise EarendelError(400, "room_version must be a string", Codes.BAD_JSON)
        ...
        if "room_alias_name" in config:
            for wchar in string.whitespace:
                if wchar in config["room_alias_name"]:
                    raise EarendelError(400, "Invalid characters in room alias")
            ...
            if mapping:
                raise EarendelError(400, "Room alias already taken", Codes.ROOM_IN_USE)
        ...
        for i in invite_list:
            try:
                ...
                raise EarendelError(400, "Invalid user_id: %s" % (i,))
        ...
        if invite_list or invite_3pid_list:
            try:
                ...
                raise EarendelError(400, "Cannot invite so many users at once")
        ...
        if not allowed_by_third_party_rules:
            raise EarendelError(403, "Room visibility value not allowed.")
    ...
    ) -> tuple[EventBase, earendel.events.snapshot.EventContext]:
    ...
    async def _send_events_for_new_room(
        ...
        async def create_event(
            ...
            for_batch: bool,
            **kwargs: Any,
        ) -> tuple[EventBase, earendel.events.snapshot.UnpersistedEventContextBase]:
            """
    ...
    def _generate_room_id(self) -> str:
        ...
        Earendel's room IDs are 18 [a-zA-Z] characters long, which comes out to around
        102 bits.
...
class TimestampLookupHandler:
    ...
    async def get_event_for_timestamp(
        ...
        Raises:
            EarendelError if unable to find any event locally in the given direction
        """
        logger.debug(
...
class RoomShutdownHandler:
    ...
    async def shutdown_room(
        ...
        if not RoomID.is_valid(room_id):
            raise EarendelError(400, "%s is not a legal room ID" % (room_id,))
