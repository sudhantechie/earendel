...
from earendel.types import UserID, map_username_to_mxid_localpart
    ...
    def _parse_cas_response(self, cas_response_body: bytes) -> CasResponse:
        ...
        for child in root[0]:
            if child.tag.endswith("user"):
                user = child.text
                # if numeric user IDs are allowed and username is numeric then we add the prefix so Earendel can handle it
