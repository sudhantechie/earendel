...
from earendel.crypto.keyring import VerifyJsonRequest
...
class RoomPolicyHandler:
    ...
    def _is_policy_server_state_event(self, event: EventBase) -> bool:
        ...
        if state_key is not None and state_key == "":
            ...
            # https://github.com/sudhantechie/earendel/issues/19502
    ...
    async def _get_policy_server(self, room_id: str) -> PolicyServerInfo | None:
        ...
        - The via server is Earendel itself.

        TODO: Remove unstable MSC4284 support - https://github.com/sudhantechie/earendel/issues/19502
        ...
        if policy_server == self._hs.hostname:
            return None  # Earendel itself can't be a policy server (currently)
    ...
    async def is_event_allowed(self, event: EventBase) -> bool:
        ...
        Note: This will *always* return True if the room's policy server is Earendel
        itself. This is because Earendel can't be a policy server (currently).
    ...
    async def ask_policy_server_to_sign_event(
        ...
        try:
            ...
            # logically the same server, but run by different software. For example, Earendel
            ...
            # re-wrap HTTP errors as `EarendelError` so they can be proxied to clients directly
            raise ex.to_earendel_error() from ex
