...
from earendel.api.constants import MAX_ALIAS_LENGTH, EventTypes
...
from earendel.types import JsonDict, Requester, RoomAlias
...
class DirectoryHandler:
    ...
    async def _create_association(
        ...
        if ":" in room_alias.localpart:
            raise EarendelError(400, "Invalid character in room alias localpart: ':'.")

        if not self.hs.is_mine(room_alias):
            raise EarendelError(400, "Room alias must be local")
        ...
        if not servers:
            raise EarendelError(400, "Failed to get server list")
    ...
    async def create_association(
        ...
        if service:
            ...
            if not self.config.roomdirectory.is_alias_creation_allowed(
                ...
                raise EarendelError(403, "Not allowed to create alias")
    ...
    async def delete_association(
        ...
        Raises:
            ...
            EarendelError: if the alias belongs to an AS
        """
        user_id = requester.user.to_string()
    ...
    async def get_association(self, room_alias: RoomAlias) -> JsonDict:
        ...
        if self.hs.is_mine(room_alias):
            ...
            try:
                ...
                raise EarendelError(502, "Failed to fetch alias")
    ...
    async def on_directory_query(self, args: JsonDict) -> JsonDict:
        ...
        if not self.hs.is_mine(room_alias):
            raise EarendelError(400, "Room Alias is not hosted on this homeserver")
    ...
    async def edit_published_room_list(
        ...
        if room is None:
            raise EarendelError(400, "Unknown room")
        ...
        if making_public:
            ...
            if not self.config.roomdirectory.is_publishing_room_allowed(
                ...
                # reasons why we said no. TODO: Allow configurable error messages
                # per alias creation rule?
                raise EarendelError(403, "Not allowed to publish room")
