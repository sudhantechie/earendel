...
import earendel.metrics._reactor_metrics  # noqa: F401
from earendel.metrics._gc import MIN_TIME_BETWEEN_GCS, install_gc_manager
from earendel.metrics._types import Collector
from earendel.earendel_rust import get_rustc_version
from earendel.types import StrSequence
...
to. Because we support multiple instances of Earendel running in the same process and all
metrics are in a single global `REGISTRY`, we need to manually label any metrics.

In the case of a Earendel homeserver, this should be set to the homeserver name
(`hs.hostname`).
    ...
    def register_hook(
        ...
        *,
        ...
        assert existing_hook is None, (
            ...
            "This is likely a Earendel bug and you forgot to unregister the previous hooks for "
...
sent_transactions_counter = Counter(
    "earendel_federation_client_sent_transactions", "", labelnames=[SERVER_NAME_LABEL]
)

events_processed_counter = Counter(
    "earendel_federation_client_events_processed", "", labelnames=[SERVER_NAME_LABEL]
)

event_processing_loop_counter = Counter(
    "earendel_event_processing_loop_count",
    "Event processing loop iterations",
    labelnames=["name", SERVER_NAME_LABEL],
)

event_processing_loop_room_count = Counter(
    "earendel_event_processing_loop_room_count",
...
event_processing_positions = Gauge(
    "earendel_event_processing_positions", "", labelnames=["name", SERVER_NAME_LABEL]
)

# Used to track the current max events stream position
event_persisted_position = Gauge(
    "earendel_event_persisted_position", "", labelnames=[SERVER_NAME_LABEL]
)

# Used to track the received_ts of the last event processed by various
# components
event_processing_last_ts = Gauge(
    "earendel_event_processing_last_ts", "", labelnames=["name", SERVER_NAME_LABEL]
)
...
event_processing_lag = Gauge(
    "earendel_event_processing_lag", "", labelnames=["name", SERVER_NAME_LABEL]
)

event_processing_lag_by_event = Histogram(
    "earendel_event_processing_lag_by_event",
...
# consider this process-level because all Earendel homeservers running in the process
# will use the same Earendel version.
...
earendel_server_name_info = Gauge(
    "earendel_server_name_info",
    "Maps Earendel `server_name`s to the `instance`s they're hosted on",
...
This is an info-style metric where the value is always 1, and labels carry metadata:
 ...
 - `server_name`: The Earendel `server_name`
...
threepid_send_requests = Histogram(
    "earendel_threepid_send_requests_with_tries",
