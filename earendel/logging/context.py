...
"""Thread-local-alike tracking of log contexts within earendel

This module provides objects and utilities for tracking contexts through
earendel code, so that log lines can include a request identifier, and so that
...
from earendel.logging.loggers import ExplicitlyConfiguredLogger
from earendel.util.stringutils import random_string_insecure_fast

if TYPE_CHECKING:
    from earendel.logging.scopecontextmanager import _LogContextScope
...
logcontext_debug_logger = logging.getLogger("earendel.logging.context.debug")
"""
...
in the logs. Requires explicitly setting `earendel.logging.context.debug` in the logging
...
class ContextRequest:
    """
    A bundle of attributes from the EarendelRequest object.

    This exists to:

    * Avoid a cycle between LoggingContext and EarendelRequest.
...
class _Sentinel:
    ...
    This should only be used for tasks outside of Earendel like when we yield control
    ...
    Nothing from the Earendel homeserver should be logged with the sentinel context. i.e.
...
class LoggingContext:
    ...
    Args:
        ...
        request: Earendel Request Context object. Useful to associate all the logs
...
class LoggingContextFilter(logging.Filter):
    ...
    def __init__(
        ...
        # filters:
        #   context:
        #       (): earendel.logging.context.LoggingContextFilter
        #       request: ""
        # ```
        #
    ...
    def filter(self, record: logging.LogRecord) -> Literal[True]:
        ...
        Please be mindful of 3rd-party code outside of Earendel (like in the case of
        Earendel Pro for small hosts) as this is running as a global log record filter.
        Other code may have set their own attributes on the record and the log record
        may not be relevant to Earendel at all so we should not mangle it.

        We can have some defaults but we should avoid overwriting existing attributes on
        any log record unless we actually have a Earendel logcontext (not just the
        ...
        # from the logcontext.
            ...
            def safe_set(attr: str, value: Any) -> None:
                ...
                a Earendel logcontext (indicating that this log record is relevant to
                Earendel).
                """
            ...
            # know we have some actual Earendel logcontext and don't need to worry about
...
def run_in_background(
    ...
    **kwargs: P.kwargs,
    ...
    if d.called and not d.paused:
        # If the function messes with logcontexts, we can assume it follows the Earendel
    ...
    # Since the function we called may follow the Earendel logcontext rules (Rules for
    ...
    # If the function we called is playing nice and following the Earendel logcontext
...
def make_deferred_yieldable(deferred: "defer.Deferred[T]") -> "defer.Deferred[T]":
    """
    Given a deferred, make it follow the Earendel logcontext rules:
...
def defer_to_thread(
    reactor: "IEarendelReactor", f: Callable[P, R], *args: P.args, **kwargs: P.kwargs
) -> "defer.Deferred[R]":
    """
    ...
    The result deferred follows the Earendel logcontext rules: you should `yield`
    on it.
