...
from earendel.config._util import validate_config
...
DEFAULT_USER_MAPPING_PROVIDER = "earendel.handlers.oidc.JinjaOidcMappingProvider"
# The module that JinjaOidcMappingProvider is in was renamed, we want to
# transparently handle both the same.
LEGACY_USER_MAPPING_PROVIDER = "earendel.handlers.oidc_handler.JinjaOidcMappingProvider"
...
class OIDCConfig(Config):
    ...
    def read_config(self, config: JsonDict, **kwargs: Any) -> None:
        ...
        # after earendel has forked so doesn't give nice errors.)
...
class OidcProviderConfig:
    ...
    # Whether Earendel should react to backchannel logouts
    backchannel_logout_enabled: bool

    # Whether Earendel should ignore the `sub` claim in backchannel logouts or not.
    ...
    """
    An optional replacement for Earendel's hardcoded `redirect_uri` URL
    ...
    If this is set, the client is expected to call Earendel's OIDC callback URL
