...
from earendel.api.constants import RoomCreationPreset
from earendel.config._base import Config, ConfigError, read_file
from earendel.types import JsonDict, RoomAlias, UserID
from earendel.util.stringutils import random_string_with_symbols, strtobool
...
password resets, configure Earendel with an SMTP server via the `email` setting, and
            ...
            if self.auto_join_room_requires_invite:
                if not mxid_localpart:
                    raise ConfigError(
                        ...
                        "Earendel knows who to send invitations from. Please "
