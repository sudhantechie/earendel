...
from earendel.config.server import generate_ip_set, parse_proxy_config
from earendel.types import JsonDict
from earendel.util.check_dependencies import check_requirements
...
The Earendel config url_preview_ip_range_blacklist will be ignored as an HTTP(s) proxy is configured."""
    ...
    for size in thumbnail_sizes:
        ...
        for format, thumbnail_format in THUMBNAIL_SUPPORTED_MEDIA_FORMAT_MAP.items():
            ...
            if thumbnail_format == "jpeg":
                ...
                raise Exception(
                    "Unknown thumbnail mapping from %s to %s. This is a Earendel problem, please report!"
...
class MediaUploadLimit:
    ...
    These can be configured through the `media_upload_limits` [config option](https://github.com/sudhantechie/earendel/latest/usage/configuration/config_documentation.html#media_upload_limits)
    or via the `get_media_upload_limits_for_user` module API [callback](https://github.com/sudhantechie/earendel/latest/modules/media_repository_callbacks.html#get_media_upload_limits_for_user).
    """
    ...
    If left unset (`None`), Earendel falls back to a static page served by itself
...
class ContentRepositoryConfig(Config):
    ...
    def read_config(self, config: JsonDict, **kwargs: Any) -> None:
        ...
        if (
            ...
            and config.get("worker_app") != "earendel.app.media_repository"
        ):
        ...
        for i, provider_config in enumerate(storage_providers):
            ...
            if (
                ...
                or provider_config["module"] == "earendel.rest.media.v1.storage_provider"
            ):
                provider_config["module"] = (
                    "earendel.media.storage_provider.FileStorageProviderBackend"
                )
        ...
        # module load time.
        from earendel.rest.earendel.client.media_upload_limit_exceeded import (
