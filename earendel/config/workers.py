...
from earendel.config._base import (
    Config,
    ConfigError,
...
from earendel.config._util import parse_and_validate_mapping
...
documentation here:
`https://github.com/sudhantechie/earendel/latest/usage/configuration/config_documentation.html#instance_map`
"""
...
`https://github.com/sudhantechie/earendel/latest/workers.html#worker-configuration`
"""
...
class WorkerConfig(Config):
    """The workers are processes run separately to the main earendel process.
    They have their own pid_file and listener configuration. They use the
    replication_url to talk to the main earendel process."""
    ...
    def read_config(
        ...
        if self.worker_app == "earendel.app.homeserver":
        ...
        # The port on the main earendel for TCP replication
        if "worker_replication_port" in config:
            raise ConfigError(DIRECT_TCP_ERROR, ("worker_replication_port",))

        # The shared secret used for authentication when connecting to the main earendel.
        ...
        if self.worker_main_http_uri is not None:
            logger.warning(
                "The config option worker_main_http_uri is unused since Earendel 1.73. "
        ...
        if self.instance_name is not MAIN_PROCESS_INSTANCE_NAME:
            ...
            # The host used to connect to the main earendel
            ...
            # The port on the main earendel for HTTP replication endpoint
            ...
            # The tls mode on the main earendel for HTTP replication endpoint.
        ...
        self.should_notify_appservices = self._should_this_worker_perform_duty(
            ...
            legacy_worker_app_name="earendel.app.appservice",
        ...
        self.should_update_user_directory = self._should_this_worker_perform_duty(
            ...
            legacy_worker_app_name="earendel.app.user_dir",
    ...
    def _should_this_worker_perform_duty(
        ...
        Parameters:
            ...
            legacy_worker_app_name: The name of a legacy Earendel worker application
                that would traditionally perform this duty.
                e.g. "earendel.app.appservice"
        ...
        if self.worker_app == legacy_worker_app_name and config.get(
            ...
            raise ConfigError(
                ...
                f"Consider setting `worker_app: earendel.app.generic_worker` and using the '{new_option_name}' option instead.\n"
        ...
        if (
            ...
            if new_option_should_run_here != legacy_option_should_run_here:
                update_worker_type = (
                    " and set worker_app: earendel.app.generic_worker"
    ...
    def _worker_names_performing_this_duty(
        ...
        1. The old way where "start_pushers" is set to false and running a
          `earendel.app.pusher'` worker app.
        ...
        Args:
            ...
            legacy_app_name: The historical app name. e.g. 'earendel.app.pusher'
            ...
            if self.worker_app == legacy_app_name:
                if legacy_option:
                    ...
                    raise ConfigError(
                        ...
                        "the main earendel process before they can be run in a separate "
                        "worker.\n"
