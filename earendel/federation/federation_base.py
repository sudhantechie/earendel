...
from earendel.api.constants import MAX_DEPTH, EventContentFields, EventTypes, Membership
...
from earendel.crypto.event_signing import check_event_content_hash
from earendel.crypto.keyring import Keyring
from earendel.events import EventBase, make_event_from_dict
from earendel.events.utils import prune_event, validate_canonicaljson
from earendel.federation.units import filter_pdus_for_valid_depth
from earendel.handlers.room_policy import RoomPolicyHandler
from earendel.http.servlet import assert_params_in_dict
from earendel.logging.opentracing import log_kv, trace
...
def event_from_pdu_json(
    ...
    Raises:
        EarendelError: if the pdu is missing required fields or is otherwise
    ...
    if "unsigned" in pdu_json:
        ...
        # what Earendel uses internally. We also remove the `age` field to avoid
        # confusion.
        #
        # c.f. https://github.com/sudhantechie/earendel/issues/8429
    ...
    if type(depth) is not int:  # noqa: E721
        raise EarendelError(400, "Depth %r not an intger" % (depth,), Codes.BAD_JSON)

    if depth < 0:
        raise EarendelError(400, "Depth too small", Codes.BAD_JSON)
    elif depth > MAX_DEPTH:
        raise EarendelError(400, "Depth too large", Codes.BAD_JSON)
