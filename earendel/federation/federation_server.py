...
from earendel.events.snapshot import EventPersistencePair
from earendel.federation.federation_base import (
    FederationBase,
    InvalidEventSignatureError,
    event_from_pdu_json,
)
from earendel.federation.persistence import TransactionActions
from earendel.federation.units import Edu, Transaction, serialize_and_filter_pdus
from earendel.handlers.worker_lock import NEW_EVENT_DURING_PURGE_LOCK_NAME
...
from earendel.metrics.background_process_metrics import wrap_as_background_process
from earendel.module_api.callbacks.federation import FederatedEventDeliveryMethod
from earendel.replication.http.federation import (
    ReplicationFederationSendEduRestServlet,
)
from earendel.storage.databases.main.lock import Lock
from earendel.storage.databases.main.roommember import extract_heroes_from_room_summary
from earendel.storage.roommember import MemberSummary
from earendel.types import JsonDict, StateMap, UserID, get_domain_from_id
from earendel.util import unwrapFirstError
from earendel.util.async_helpers import Linearizer, concurrently_execute, gather_results
from earendel.util.caches.response_cache import ResponseCache
from earendel.util.duration import Duration
from earendel.util.stringutils import parse_server_name
...
received_pdus_counter = Counter(
    "earendel_federation_server_received_pdus", "", labelnames=[SERVER_NAME_LABEL]
)

received_edus_counter = Counter(
    "earendel_federation_server_received_edus", "", labelnames=[SERVER_NAME_LABEL]
)

received_queries_counter = Counter(
    "earendel_federation_server_received_queries",
    "",
    labelnames=["type", SERVER_NAME_LABEL],
)

pdu_process_time = Histogram(
    "earendel_federation_server_pdu_process_time",
    "Time taken to process an event",
    labelnames=[SERVER_NAME_LABEL],
)

last_pdu_ts_metric = Gauge(
    "earendel_federation_last_received_pdu_time",
...
class FederationServer(FederationBase):
    ...
    async def _handle_edus_in_txn(self, origin: str, transaction: Transaction) -> None:
        ...
        async def _process_edu(edu_dict: JsonDict) -> None:
            ...
            try:
                ...
                #
                # We log the exception now, and then raise a new EarendelError to cause
                # the transaction to be failed.
                logger.exception("Error handling EDU of type %s", edu.edu_type)
                raise EarendelError(500, f"Error handing EDU of type {edu.edu_type}")
    ...
    async def on_get_extremities_request(self, origin: str, room_id: str) -> JsonDict:
        ...
        if len(prev_event_ids) == 0:
            raise EarendelError(500, "Room has no forward extremities")
    ...
    async def on_invite_request(
        ...
        if await self._spam_checker_module_callbacks.should_drop_federated_event(pdu):
            ...
            raise EarendelError(403, Codes.FORBIDDEN)
        try:
            pdu = await self._check_sigs_and_hash(room_version, pdu)
        except InvalidEventSignatureError as e:
            errmsg = f"event id {pdu.event_id}: {e}"
            logger.warning("%s", errmsg)
            raise EarendelError(403, errmsg, Codes.FORBIDDEN)
    ...
    async def on_send_join_request(
        ...
        set_tag(
            EarendelTags.SEND_JOIN_RESPONSE_IS_PARTIAL_STATE,
        ...
        if supports_msc4242_state_dag(event):
            # FIXME: We don't yet support faster room joins in Earendel with MSC4242
    ...
    async def on_make_knock_request(
        ...
        if await self.store.is_partial_state_room(room_id):
            ...
            # block due to https://github.com/sudhantechie/earendel/issues/12997.
            raise EarendelError(
                404,
    ...
    async def _on_send_membership_event(
        ...
        Raises:
            EarendelError if there is a problem with the request, including things like
        ...
        if event.type != EventTypes.Member or not event.is_state():
            raise EarendelError(400, "Not an m.room.member event", Codes.BAD_JSON)

        if event.content.get("membership") != membership_type:
            raise EarendelError(400, "Not a %s event" % membership_type, Codes.BAD_JSON)
    ...
    async def on_claim_client_keys(
        ...
        if any(
            ...
            raise EarendelError(400, "User is not hosted on this homeserver")
