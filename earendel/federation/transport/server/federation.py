...
from earendel.api.constants import Direction, EduTypes, StateDag
from earendel.api.errors import Codes, EarendelError
from earendel.api.room_versions import RoomVersions
from earendel.api.urls import FEDERATION_UNSTABLE_PREFIX, FEDERATION_V2_PREFIX
from earendel.federation.transport.server._base import (
    Authenticator,
...
from earendel.media._base import DEFAULT_MAX_TIMEOUT_MS, MAXIMUM_ALLOWED_MAX_TIMEOUT_MS
from earendel.media.thumbnailer import ANIMATED_THUMBNAIL_TYPE, ThumbnailProvider
...
class FederationV2InviteServlet(BaseFederationServerServlet):
    ...
    async def on_PUT(
        ...
        # Earendel expects invite_room_state to be in unsigned, as it is in v1
        # API
            ...
            for invite in content["invites"]:
                try:
                    if "signed" not in invite or "token" not in invite["signed"]:
                        ...
                        logger.info(message)
                        raise EarendelError(400, message)
...
class RoomComplexityServlet(BaseFederationServlet):
    ...
    async def on_GET(
        ...
        if not is_public:
            raise EarendelError(404, "Room not found", errcode=Codes.INVALID_PARAM)
