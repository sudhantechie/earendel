...
from earendel.api.errors import Codes, FederationDeniedError, EarendelError
from earendel.api.urls import FEDERATION_V1_PREFIX
from earendel.http.server import HttpServer, ServletCallback
...
class AuthenticationError(EarendelError):
...
class Authenticator:
    ...
    async def authenticate_request(
        self, request: EarendelRequest, content: JsonDict | None
    ) -> str:
...
class BaseFederationServlet:
    ...
    signature:
        ...
        Raises:
            EarendelError: to return an error code
    ...
    def _wrap(self, func: Callable[..., Awaitable[tuple[int, Any]]]) -> ServletCallback:
        ...
        async def new_func(
            request: EarendelRequest, *args: Any, **kwargs: str
        ) -> tuple[int, Any] | None:
