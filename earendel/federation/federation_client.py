...
from earendel.api.constants import Direction, EventContentFields, EventTypes, Membership
...
from earendel.events import EventBase, builder, make_event_from_dict
...
from earendel.federation.transport.client import SendJoinResponse
from earendel.http.client import is_unknown_endpoint
from earendel.http.types import QueryParams
from earendel.logging.opentracing import EarendelTags, log_kv, set_tag, tag_args, trace
from earendel.metrics import SERVER_NAME_LABEL
from earendel.types import JsonDict, StrCollection, UserID, get_domain_from_id
...
sent_queries_counter = Counter(
    "earendel_federation_client_sent_queries", "", labelnames=["type", SERVER_NAME_LABEL]
)
...
class FederationClient(FederationBase):
    ...
    async def get_pdu_from_destination_raw(
        ...
        Raises:
            EarendelError, NotRetryingDestination, FederationDeniedError
        """
    ...
    async def get_room_state_ids(
        ...
        set_tag(
            EarendelTags.RESULT_PREFIX + "state_event_ids",
            str(state_event_ids),
        )
        set_tag(
            EarendelTags.RESULT_PREFIX + "state_event_ids.length",
            str(len(state_event_ids)),
        )
        set_tag(
            EarendelTags.RESULT_PREFIX + "auth_event_ids",
            str(auth_event_ids),
        )
        set_tag(
            EarendelTags.RESULT_PREFIX + "auth_event_ids.length",
    ...
    async def _check_sigs_and_hash_for_pulled_events_and_fetch(
        ...
        """
        set_tag(
            EarendelTags.RESULT_PREFIX + "pdus.length",
            str(len(pdus)),
        )
    ...
    async def _try_destination_list(
        ...
        Raises:
            EarendelError if the chosen remote server returns a 300/400 code, or
        ...
        for destination in destinations:
            ...
            try:
                ...
                earendel_error = e.to_earendel_error()
                failover = False
                ...
                elif 400 <= e.code < 500 and earendel_error.errcode in failover_errcodes:
                    failover = True
        ...
        raise EarendelError(502, f"Failed to {description} via any server")
        ...
        async def send_request(destination: str) -> SendJoinResult:
            ...
            if room_version.msc4242_state_dags:
                ...
                return SendJoinResult(
                    ...
                    origin=destination,
                    # The current Earendel implementation of MSC4242 does not support
    ...
    async def _do_send_invite(
        ...
        Raises:
            EarendelError: if the remote server returns an error or if the server
    ...
    async def get_public_rooms(
        ...
        Raises:
            ...
            EarendelException: M_FORBIDDEN when the remote server has disallowed publicRoom
    ...
    async def get_room_hierarchy(
        ...
        Raises:
            EarendelError if we were unable to get a valid summary from any of the
               remote servers
        """
