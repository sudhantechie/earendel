...
if typing.TYPE_CHECKING:
    ...
    from earendel.types import JsonDict, StrCollection
...
class EarendelError(CodeMessageException):
    ...
    probably shouldn't be a `EarendelError`. For example, if we contact another
...
class InvalidAPICallError(EarendelError):
...
class InvalidProxyCredentialsError(EarendelError):
...
class ProxiedRequestError(EarendelError):
...
class ConsentNotGivenError(EarendelError):
...
class UserDeactivatedError(EarendelError):
...
class UserLockedError(EarendelError):
    """The error returned to the client when the user attempted to access an
...
class FederationDeniedError(EarendelError):
...
class UnrecognizedRequestError(EarendelError):
...
class OAuthInsufficientScopeError(EarendelError):
...
class InvalidClientCredentialsError(EarendelError):
...
class ResourceLimitError(EarendelError):
    """
...
class UserLimitExceededError(EarendelError):
    """
...
class EventSizeError(EarendelError):
...
class LimitExceededError(EarendelError):
...
class RoomKeysVersionError(EarendelError):
...
class UnsupportedRoomVersionError(EarendelError):
...
class ThreepidValidationError(EarendelError):
...
class IncompatibleRoomVersionError(EarendelError):
...
class PasswordRefusedError(EarendelError):
...
class UnredactedContentDeletedError(EarendelError):
...
class NotApprovedError(EarendelError):
    def __init__(
        self,
        msg: str,
...
class HttpResponseException(CodeMessageException):
    ...
    def unsafe_to_verbatim_earendel_error(self) -> EarendelError:
        """Make a EarendelError directly based on a TRUSTED HTTPResponseException.
        ...
        are copied verbatim into the new Earendel error.
        ...
        Safety:
            ...
            such as other Earendel workers.

        Returns:
            The error converted to a EarendelError.
        """
    ...
    def to_earendel_error(self) -> EarendelError:
        """Make a EarendelError directly based on a TRUSTED HTTPResponseException.
        ...
        An attempt is made to parse the body of the HTTP response as a Matrix
        error. If that succeeds, the errcode and error message from the body
        are copied verbatim into the new Earendel error, unless it's of
        a forbidden type.
        ...
        if errcode == Codes.UNKNOWN_TOKEN:
            ...
            # See: https://github.com/sudhantechie/earendel/security/advisories/GHSA-95fh-hv8c-chvq
            errcode = Codes.UNKNOWN
...
class PartialStateConflictError(EarendelError):
...
class SlidingSyncUnknownPosition(EarendelError):
    """An error that Earendel can return to signal to the client to expire their
