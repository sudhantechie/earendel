...
from earendel.api.errors import Codes, InvalidProxyCredentialsError
from earendel.http import QuieterFileBodyProducer
from earendel.http.server import _AsyncResource
...
class ProxyResource(_AsyncResource):
    ...
    def __init__(self, reactor: IEarendelReactor, hs: "HomeServer"):
...
class _ProxyResponseBody(protocol.Protocol):
    ...
    def __init__(self, request: "EarendelRequest") -> None:
...
class ProxySite(Site):
    ...
    def getResourceFor(self, request: "EarendelRequest") -> IResource:
        uri = urllib.parse.urlparse(request.uri)
