...
from earendel.api.errors import Codes, HttpResponseException, EarendelError
from earendel.http import QuieterFileBodyProducer, RequestTimedOutError, redact_uri
from earendel.http.proxyagent import ProxyAgent
from earendel.http.replicationagent import ReplicationAgent
...
from earendel.logging.opentracing import set_tag, start_active_span, tags
from earendel.metrics import SERVER_NAME_LABEL
from earendel.types import IEarendelReactor, StrSequence
from earendel.util.async_helpers import timeout_deferred
from earendel.util.clock import CLOCK_SCHEDULE_EPSILON, Clock
...
outgoing_requests_counter = Counter(
    "earendel_http_client_requests", "", labelnames=["method", SERVER_NAME_LABEL]
)
incoming_responses_counter = Counter(
    "earendel_http_client_responses",
    "",
...
# IEarendelReactor implies IReactorCore, but explicitly marking it this as an implementer
# of IReactorCore seems to keep mypy-zope happier.
@implementer(IReactorCore, IEarendelReactor)
    ...
    def request(
        ...
        try:
            ...
            if _is_ip_blocked(ip_address, self._ip_allowlist, self._ip_blocklist):
                ...
                e = EarendelError(HTTPStatus.FORBIDDEN, "IP address blocked")
...
class BaseHttpClient:
    ...
    async def get_file(
        ...
        Raises:
            ...
            EarendelError: if the response is not a 2xx, the remote file is too large, or
...
class SimpleHttpClient(BaseHttpClient):
    ...
    def __init__(
        ...
        if self._ip_blocklist:
            ...
            self.reactor: IEarendelReactor = BlocklistingReactorWrapper(
...
def is_unknown_endpoint(
    e: HttpResponseException, earendel_error: EarendelError | None = None
) -> bool:
    """
    ...
    Args:
        ...
        earendel_error: The above error converted to a EarendelError. This is
            automatically generated if not provided.

    """
    if earendel_error is None:
    ...
    return (
        ...
        and (
            ...
            or earendel_error.errcode == Codes.UNRECOGNIZED
        )
    ) or (
        # Older Earendels returned a 400 error.
        e.code == 400 and earendel_error.errcode == Codes.UNRECOGNIZED
    )
