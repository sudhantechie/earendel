...
from earendel.http import redact_uri
from earendel.http.server import HttpServer
from earendel.types import JsonDict, RoomAlias, RoomID, StrCollection
...
def parse_integer(
    ...
    Args:
        ...
        required: whether to raise a 400 EarendelError if the parameter is absent,
            defaults to False.
    ...
    Raises:
        EarendelError: if the parameter is absent and required, if the
...
def parse_integer_from_args(
    ...
    if name_bytes not in args:
        ...
        raise EarendelError(HTTPStatus.BAD_REQUEST, message, errcode=Codes.MISSING_PARAM)

    try:
        integer = int(args[name_bytes][0])
    except Exception:
        message = f"Query parameter {name} must be an integer"
        raise EarendelError(HTTPStatus.BAD_REQUEST, message, errcode=Codes.INVALID_PARAM)
...
def parse_boolean(
    ...
    Raises:
        EarendelError: if the parameter is absent and required, or if the
...
def parse_bytes_from_args(
    ...
    Args:
        ...
        required: whether to raise a 400 EarendelError if the
            parameter is absent, defaults to False.
    Returns:
        Bytes or the default value.

    Raises:
        EarendelError if the parameter is absent and required.
    """
    name_bytes = name.encode("ascii")
...
def parse_json_from_args(
    ...
    try:
        ...
        raise EarendelError(HTTPStatus.BAD_REQUEST, message, errcode=Codes.NOT_JSON)
...
def parse_json_value_from_request(
    ...
    Raises:
        EarendelError if the request body couldn't be decoded as JSON.
    """
    try:
        content_bytes = request.content.read()  # type: ignore
    except Exception:
        raise EarendelError(HTTPStatus.BAD_REQUEST, "Error reading JSON content.")
...
def parse_json_object_from_request(
    ...
    Raises:
        EarendelError if the request body couldn't be decoded as JSON or
...
def validate_json_object(content: JsonDict, model_type: type[Model]) -> Model:
    ...
    try:
        ...
        raise EarendelError(HTTPStatus.BAD_REQUEST, str(e), errcode=errcode)
...
class ResolveRoomIdMixin:
    ...
    async def resolve_room_id(
        ...
        Raises:
            EarendelError if the room ID is of the wrong form.
        """
