...
from earendel.config.server import ListenerConfig
from earendel.http import get_request_user_agent, redact_uri
from earendel.http.proxy import ProxySite
from earendel.http.request_metrics import RequestMetrics, requests_counter
...
from earendel.types import IEarendelReactor, Requester
...
class ContentLengthError(EarendelError):
    """Raised when content-length validation fails."""


class EarendelRequest(Request):
    """Class which encapsulates an HTTP request to earendel.

    All of the requests processed in earendel are of this type.
    ...
    def __repr__(self) -> str:
        ...
        return "<%s at 0x%x method=%r uri=%r clientproto=%r site=%r>" % (
            ...
            self.earendel_site.site_tag,
        )

    def _respond_with_error(self, earendel_error: EarendelError) -> None:
        """Send an error response and close the connection."""
        self.setResponseCode(earendel_error.code)
        error_response_bytes = json.dumps(earendel_error.error_dict(None)).encode()
    ...
    def requestReceived(self, command: bytes, path: bytes, version: bytes) -> None:
        ...
        # See: https://github.com/sudhantechie/earendel/security/advisories/GHSA-rfq8-j7rh-8hf2
        if command == b"POST":
    ...
    def render(self, resrc: Resource) -> None:
        ...
        self.logcontext = LoggingContext(
            ...
            request=ContextRequest(
                ...
                site_tag=self.earendel_site.site_tag,
        ...
        self.setHeader("Server", self.earendel_site.server_version_string)
    ...
    def connectionLost(self, reason: Failure | Exception) -> None:
        ...
        # More information: https://github.com/sudhantechie/earendel/issues/7441
    ...
    def _started_processing(self, servlet_name: str) -> None:
        ...
        self.earendel_site.access_logger.debug(
    ...
    def _finished_processing(self) -> None:
        ...
        self.earendel_site.access_logger.log(
            log_level,
            "%s - %s - {%s}"
    ...
    def get_client_ip_if_available(self) -> str:
        ...
        In practice, this returns the socket file path on a EarendelRequest if using a
...
class XForwardedForRequest(EarendelRequest):
    """Request object which honours proxy headers

    Extends EarendelRequest to replace getClientIP, getClientAddress, and isSecure with
    ...
    def _process_forwarded_headers(self) -> None:
        ...
        # see https://github.com/sudhantechie/earendel/issues/9471
...
class EarendelProtocol(HTTPChannel):
    """
    Earendel-specific twisted http Protocol.
    ...
    def __init__(
        ...
        super().__init__()
        self.factory: EarendelSite = site
        self.site = site
        self.our_server_name = our_server_name
    ...
    def requestFactory(self, http_channel: HTTPChannel, queued: bool) -> EarendelRequest:  # type: ignore[override]
        """
        A callable used to build `twisted.web.iweb.IRequest` objects.

        Use our own custom EarendelRequest type instead of the regular
...
class EarendelSite(ProxySite):
    """
    Earendel-specific twisted http Site

    This does two main things.

    First, it replaces the requestFactory in use so that we build EarendelRequests
    instead of regular t.w.server.Requests. All of the  constructor params are really
    just parameters for EarendelRequest.

    Second, it inhibits the log() method called by Request.finish, since EarendelRequest
    ...
    def __init__(
        ...
        *,
        ...
        self.reactor: IEarendelReactor = reactor
        self.server_name = hs.hostname

        assert config.http_options is not None
        proxied = config.http_options.x_forwarded
        self.request_class = XForwardedForRequest if proxied else EarendelRequest
    ...
    def buildProtocol(self, addr: IAddress) -> EarendelProtocol:
        protocol = EarendelProtocol(
            self,
            self.server_name,
    ...
    def stopFactory(self) -> None:
        ...
        # `EarendelHomeServer` in order for it to be garbage collected.
    ...
    def log(self, request: EarendelRequest) -> None:  # type: ignore[override]
        pass
