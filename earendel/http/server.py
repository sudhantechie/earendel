...
from earendel.types import IEarendelThreadlessReactor
...
from earendel.logging.context import defer_to_thread, preserve_fn, run_in_background
from earendel.logging.opentracing import active_span, start_active_span, trace_servlet
from earendel.util.caches import intern_dict
from earendel.util.cancellation import is_function_cancellable
from earendel.util.clock import Clock
from earendel.util.duration import Duration
from earendel.util.iterutils import chunk_seq
...
def return_json_error(
    f: failure.Failure, request: "EarendelRequest", config: HomeServerConfig | None
) -> None:
    """Sends a JSON error response to clients."""

    if f.check(EarendelError):
        # mypy doesn't understand that f.check asserts the type.
        exc: EarendelError = f.value
        error_code = exc.code
        ...
        if error_ctx:
            logger.info(
                "%s EarendelError: %s - %s (%s)", request, error_code, exc.msg, error_ctx
            )
        else:
            logger.info("%s EarendelError: %s - %s", request, error_code, exc.msg)
...
def return_html_error(
    ...
    Handles RedirectException and other CodeMessageExceptions (such as EarendelError)
    ...
    if f.check(CodeMessageException):
        ...
        elif isinstance(cme, EarendelError):
            logger.info("%s EarendelError: %s - %s", request, code, msg)
        else:
            logger.error(
...
def wrap_async_request_handler(
    h: Callable[["_AsyncResource", "EarendelRequest"], Awaitable[None]],
) -> Callable[["_AsyncResource", "EarendelRequest"], "defer.Deferred[None]"]:
    ...
    where "request" must be a EarendelRequest.
    ...
    async def wrapped_async_request_handler(
        self: "_AsyncResource", request: "EarendelRequest"
    ) -> None:
...
# it is actually called with a EarendelRequest and a kwargs dict for the params,
...
class _AsyncResource(resource.Resource, metaclass=abc.ABCMeta):
    ...
    def render(self, request: "EarendelRequest") -> int:
    ...
    async def _async_render_wrapper(self, request: "EarendelRequest") -> None:
    ...
    async def _async_render(self, request: "EarendelRequest") -> tuple[int, Any] | None:
...
class DirectServeJsonResource(_AsyncResource):
    ...
    def __init__(
        ...
        Args:
            ...
            clock: This is expected to be passed in by any Earendel code.
        ...
        if clock is None:
            ...
            # As of the time of writing this, all Earendel internal usages of
            # `DirectServeJsonResource` pass in the existing homeserver clock instance.
            clock = Clock(  # type: ignore[multiple-internal-clocks]
                cast(IEarendelThreadlessReactor, reactor),
                server_name="earendel_module_running_from_unknown_server",
            )
...
class JsonResource(DirectServeJsonResource):
    ...
    async def _async_render(self, request: "EarendelRequest") -> tuple[int, Any]:
...
class OptionsResource(resource.Resource):
    ...
    def render_OPTIONS(self, request: "EarendelRequest") -> bytes:
...
def set_cors_headers(request: "EarendelRequest") -> None:
    ...
    if request.path is not None and (
        ...
        request.setHeader(
            ...
            b"Earendel-Trace-Id, Server, ETag",
        )
    else:
        request.setHeader(
            b"Access-Control-Allow-Headers",
...
def respond_with_redirect(
    request: "EarendelRequest", url: bytes, statusCode: int = FOUND, cors: bool = False
) -> None:
    """
