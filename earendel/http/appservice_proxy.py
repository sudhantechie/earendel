...
from earendel.http.proxy import (
    HOP_BY_HOP_HEADERS_LOWERCASE,
    _ProxyResponseBody,
    parse_connection_header_value,
)
from earendel.http.server import return_json_error, set_cors_headers
...
def has_dot_segments(path: bytes) -> bool:
    ...
    The caller is expected to reject such a path rather than rewrite it. Earendel
...
def _send_response(request: EarendelRequest, response: IResponse) -> None:
