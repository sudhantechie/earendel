...
from earendel.earendel_rust.types import Requester
...
if TYPE_CHECKING:
    ...
    from earendel.appservice.api import ApplicationService
    from earendel.events import EventBase
    from earendel.storage.databases.main import DataStore, PurgeEventsStore
...
class IEarendelThreadlessReactor(
    IReactorTCP,
    IReactorSSL,
    IReactorUNIX,
    IReactorPluggableNameResolver,
    ...
    """
    The interfaces necessary for Earendel to function (without threads).
...
# Note that this seems to require inheriting *directly* from Interface in order
# for mypy-zope to realize it is an interface.
class IEarendelReactor(
    IEarendelThreadlessReactor,
    IReactorThreads,
    Interface,
):
    """The interfaces necessary for Earendel to function."""
...
def get_domain_from_id(string: str) -> str:
    idx = string.find(":")
    if idx == -1:
        raise EarendelError(400, "Invalid ID: %r" % (string,))
    return string[idx + 1 :]
...
class DomainSpecificString(metaclass=abc.ABCMeta):
    ...
    def is_valid(cls: type[DS], s: str) -> bool:
        ...
        try:
            ...
            return True
        except (EarendelError, ValueError):
            return False
...
class RoomStreamToken(AbstractMultiWriterStreamToken):
    ...
    async def parse(cls, store: "PurgeEventsStore", string: str) -> "RoomStreamToken":
        # Check that it looks like a Earendel token first. We do this so that
        # we don't log at the exception-level for obviously incorrect tokens.
        if not string or string[0] not in ("s", "t", "m"):
            raise EarendelError(400, f"Invalid room stream token {string:!r}")
        ...
        try:
            ...
            # handing a bad token, its more likely that Earendel returned a bad
            # token (and we really want to catch those!).
            logger.exception("Failed to parse stream token: %r", string)
        raise EarendelError(400, "Invalid room stream token %r" % (string,))
...
class MultiWriterStreamToken(AbstractMultiWriterStreamToken):
    ...
    async def parse(cls, store: "DataStore", string: str) -> "MultiWriterStreamToken":
        try:
            ...
            # token (and we really want to catch those!).
            logger.exception("Failed to parse stream token: %r", string)
        raise EarendelError(400, "Invalid stream token %r" % (string,))
...
class StreamToken:
    ...
    async def from_string(cls, store: "DataStore", string: str) -> "StreamToken":
        ...
        try:
            ...
            raise EarendelError(400, "Invalid stream token")
...
class SlidingSyncStreamToken:
    ...
    Attributes:
        ...
        connection_position: Token used by sliding sync to track updates to any
            per-connection state stored by Earendel.
    """
...
class ThirdPartyInstanceID:
    ...
    def from_string(cls, s: str) -> "ThirdPartyInstanceID":
        bits = s.split("|", 2)
        if len(bits) != 2:
            raise EarendelError(400, "Invalid ID %r" % (s,))
