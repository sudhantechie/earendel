...
//! pair of "conditions" and "actions". When a user receives an event Earendel
...
pub fn register_module(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    // earendel.earendel_rust import push` work.
    py.import("sys")?
        .getattr("modules")?
        .set_item("earendel.earendel_rust.push", child_module)?;
...
/// The "action" Earendel should perform for a matching push rule.
...
pub struct EventMatchTypeCondition {
    ...
    // pattern property of the correct value in earendel.push.clientformat.format_push_rules_for_user.
