...
use crate::config::{types::RoomCreationPreset, EarendelHomeServerConfig};
...
/// Convert from [`EarendelHomeServerConfig`] to the global defaults for unstable features that the
/// server supports [`UnstableFeatureMap`]
pub fn earendel_config_to_global_unstable_feature_map(
    config: &EarendelHomeServerConfig,
) -> UnstableFeatureMap {
    UnstableFeatureMap {
        msc2326: true,
        msc1756: true,
