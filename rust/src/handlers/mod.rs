...
use crate::config::EarendelHomeServerConfig;
...
impl RustHandlers {
    ...
    pub fn py_new(py: Python<'_>, homeserver: &Bound<'_, PyAny>) -> PyResult<RustHandlers> {
        let config: EarendelHomeServerConfig = homeserver.getattr("config")?.extract()?;
        ...
        let global_unstable_feature_map = Arc::new(
            versions::earendel_config_to_global_unstable_feature_map(&config),
        );
...
pub fn register_module(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    py.import("sys")?
        .getattr("modules")?
        .set_item("earendel.earendel_rust.handlers", child_module)?;
