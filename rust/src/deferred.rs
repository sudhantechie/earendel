...
create_exception!(
    earendel.earendel_rust.http_client,
    RustPanicError,
    PyException,
...
/// A reference to the `earendel.logging.context` module.
static LOGGING_CONTEXT_MODULE: OnceCell<Py<PyAny>> = OnceCell::new();

/// Access to the `earendel.logging.context` module.
fn logging_context_module(py: Python<'_>) -> PyResult<&Bound<'_, PyAny>> {
    Ok(LOGGING_CONTEXT_MODULE
        .get_or_try_init(|| py.import("earendel.logging.context").map(Into::into))?
        .bind(py))
}
    ...
    for<'a> O: IntoPyObject<'a> + Send + 'static,
{
    ...
    // Make the deferred follow the Earendel logcontext rules
