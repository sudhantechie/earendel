...
/// A reference to the `earendel.util.duration` module.
static DURATION: OnceCell<Py<PyAny>> = OnceCell::new();

/// Access to the `earendel.util.duration` module.
fn duration_module(py: Python<'_>) -> PyResult<&Bound<'_, PyAny>> {
    Ok(DURATION
        .get_or_try_init(|| py.import("earendel.util.duration").map(Into::into))?
        .bind(py))
}

/// Mirrors the `earendel.util.duration.Duration` Python class.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub struct EarendelDuration {
    milliseconds: u64,
}

impl EarendelDuration {
    /// Creates a `EarendelDuration` from a number of milliseconds.
    ...
    /// Creates a `EarendelDuration` from a number of hours.
...
impl<'py> IntoPyObject<'py> for EarendelDuration {
    type Target = PyAny;
