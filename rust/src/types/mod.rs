...
//! Rust implementations of types from `earendel.types`.
...
/// A reference to the `earendel.types.UserID` class.
static USER_ID_CLASS: OnceCell<Py<PyAny>> = OnceCell::new();

/// Access to the `earendel.types.UserID` class.
fn user_id_class(py: Python<'_>) -> PyResult<&Bound<'_, PyAny>> {
    Ok(USER_ID_CLASS
        .get_or_try_init(|| -> PyResult<_> {
            Ok(py.import("earendel.types")?.getattr("UserID")?.unbind())
        })?
        .bind(py))
}
...
impl Requester {
    ...
    fn new(
        ...
        // Python bools, but unfortunately Earendel often passes them as truthy
...
pub fn register_module(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    // earendel.earendel_rust.types import Requester` work.
    py.import("sys")?
        .getattr("modules")?
        .set_item("earendel.earendel_rust.types", child_module)?;
