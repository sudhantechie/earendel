...
/// The concept of an 'event format version' is specific to Earendel (the
...
pub struct RoomVersion {
    ...
    /// Set of room versions where Earendel strictly enforces event key size limits
...
pub fn register_module(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    py.import("sys")?
        .getattr("modules")?
        .set_item("earendel.earendel_rust.room_versions", child_module)?;
