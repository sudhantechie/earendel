...
import_exception!(earendel.api.errors, EarendelError);
...
impl EarendelError {
    pub fn new(
            ...
            for (key, value) in headers.iter() {
                let Ok(value) = value.to_str() else {
                    ...
                    return PyValueError::new_err(
                        "Could not construct EarendelError: header value is not valid ASCII",
                    );
                };
        ...
        EarendelError::new_err((code.as_u16(), message, errcode, additional_fields, headers))
    }
}

import_exception!(earendel.api.errors, NotFoundError);
...
import_exception!(earendel.api.errors, HttpResponseException);
