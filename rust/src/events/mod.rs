...
//! live alongside the event JSON in Earendel:
//!
...
//! - `internal_metadata` — Earendel-internal flags that are *not* part of
...
pub fn register_module(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    // earendel.earendel_rust import events` work.
    py.import("sys")?
        .getattr("modules")?
        .set_item("earendel.earendel_rust.events", child_module)?;
...
/// that Earendel tracks outside the event JSON (event ID, internal
...
pub struct Event {
    ...
    /// Earendel-internal per-event state that lives outside the federated
...
impl Event {
    ...
    /// `None`.
    fn sticky_duration(&self) -> Option<EarendelDuration> {
        const MAX_DURATION: EarendelDuration = EarendelDuration::from_hours(1);
        ...
        let duration = EarendelDuration::from_milliseconds(duration_ms);
