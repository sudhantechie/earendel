...
/// either the Python-backed pool (in Earendel, see [`python_db_pool`]) or a Rust native
/// `tokio-postgres` pool (expected to be used in `earendel-rust-apps`).
///
            ...
            + 'static,
    {
        ...
        async move {
            ...
            boxed.downcast::<R>().map(|b| *b).map_err(|_| {
                anyhow::anyhow!(
                    "run_interaction return type mismatch (this is a Earendel programming error)"
                )
            })
        }
    }
