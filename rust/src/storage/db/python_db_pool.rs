...
/// A reference to the `earendel.storage.engines` module.
static STORAGE_ENGINES_MODULE: OnceCell<Py<PyAny>> = OnceCell::new();

/// Access to the `earendel.storage.engines` module.
fn storage_engines_module(py: Python<'_>) -> PyResult<&Bound<'_, PyAny>> {
    Ok(STORAGE_ENGINES_MODULE
        .get_or_try_init(|| py.import("earendel.storage.engines").map(Into::into))?
        .bind(py))
}
...
/// The database engines we support in the Python side of Earendel
...
/// Wrapper for a `DatabasePool` from the Python side of Earendel.
                        ...
                        match func(&mut txn).now_or_never() {
                            ...
                            None => {
                                Err(PyAssertionError::new_err(
                                    ...
                                    but we expect Earendel Python database work to resolve synchronously. \
                                    This is a Earendel programming error: genuine async work is \
                                    not supported here.",
                                ))
                            }
                        }
        ...
        match run_interaction_outcome {
            ...
            Ok(_) => match captured_result {
                ...
                None => Err(anyhow::anyhow!(
                    ...
                    This is a Earendel programming error."
                )),
            },
...
/// Earendel's transaction machinery can apply its retry logic
...
fn detect_engine(txn_py: &Bound<'_, PyAny>) -> PyResult<DatabaseEngine> {
    ...
    if database_engine.is_instance(postgres_engine_class(py)?)? {
        ...
        Err(PyTypeError::new_err(format!(
            "Unknown database engine {}. This is a Earendel programming error.",
            database_engine.get_type().name()?
        )))
    }
}

/// Wrapper for a `LoggingTransaction` from the Python side of Earendel.
